...
class BaseVoteView(discord.ui.View):
    ...
    async def finalize_vote(self, interaction: discord.Interaction, tokens_invested_this_scenario: Optional[int]):
        ...
        try:
            ...
            # Modal submissions have their own interaction, prior button interaction should be deferred.
            if not interaction.response.is_done():
                await interaction.response.defer(ephemeral=True, thinking=True)
            ...
            if self.campaign_id is not None and tokens_invested_this_scenario is not None and tokens_invested_this_scenario > 0:
                ...
                if db_update_success:
                    ...
                    print(f"CRITICAL: Vote for Proposal #{self.proposal_id} U#{self.user_id} recorded, but FAILED to update tokens for C#{self.campaign_id}. Tokens spent: {tokens_invested_this_scenario}")
