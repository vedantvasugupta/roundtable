...
async def _send_admin_approval_notification(interaction: discord.Interaction, proposal_id: int, title: str, description: str):
    """Sends a message to the admin channel to approve or reject a new proposal."""
    admin_channel_name = "proposals"  # Or from config
    admin_channel = await utils.get_or_create_channel(interaction.guild, admin_channel_name, interaction.client.user.id)

    if not admin_channel:
        print(f"ERROR: Could not find or create admin channel '{admin_channel_name}' for proposal approval.")
        # Optionally notify the user that admin notification failed
        try:
            await interaction.followup.send(
                "Your proposal was submitted, but I failed to notify the admins. Please contact them manually.",
                ephemeral=True
            )
        except discord.HTTPException:
            pass  # The original interaction might have already been responded to
        return
    ...
    # Use the existing AdminApprovalView, which is designed for single proposals
    view = AdminApprovalView(proposal_id=proposal_id)

    try:
        await admin_channel.send(
            content="Admins, a new proposal requires approval:",
            embed=embed,
            view=view
        )
    except Exception as e:
        print(f"ERROR sending proposal approval message: {e}")
        traceback.print_exc()
        try:
            await interaction.followup.send(
