...
CREATE TABLE IF NOT EXISTS users (
    ...
    discriminator TEXT, -- For Discord classic usernames
    global_name TEXT, -- For newer Discord usernames
    avatar_url TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
# Define CREATE_CONSTITUTIONAL_VARIABLES_TABLE
CREATE_CONSTITUTIONAL_VARIABLES_TABLE = """
CREATE TABLE IF NOT EXISTS constitutional_variables (
    variable_id INTEGER PRIMARY KEY AUTOINCREMENT,
    server_id INTEGER NOT NULL,
    name TEXT NOT NULL,
    value TEXT,
    type TEXT NOT NULL, -- e.g., 'boolean', 'number', 'role', 'text'
    description TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    last_updated_timestamp TIMESTAMP, -- Specific for this table as per previous fix
    UNIQUE (server_id, name),
    FOREIGN KEY (server_id) REFERENCES servers(server_id) ON DELETE CASCADE
);
"""

# Define CREATE_PROPOSAL_OPTIONS_TABLE
CREATE_PROPOSAL_OPTIONS_TABLE = """
CREATE TABLE IF NOT EXISTS proposal_options (
    option_id INTEGER PRIMARY KEY AUTOINCREMENT,
    proposal_id INTEGER NOT NULL,
    option_text TEXT NOT NULL,
    option_order INTEGER, -- To maintain order if needed
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (proposal_id, option_text), -- Prevent duplicate options for the same proposal
    FOREIGN KEY (proposal_id) REFERENCES proposals(proposal_id) ON DELETE CASCADE
);
"""

# Define CREATE_VOTES_TABLE
CREATE_VOTES_TABLE = """
CREATE TABLE IF NOT EXISTS votes (
    vote_id INTEGER PRIMARY KEY AUTOINCREMENT,
    proposal_id INTEGER NOT NULL,
    user_id INTEGER NOT NULL, -- User ID of the voter
    vote_data TEXT NOT NULL, -- JSON string, structure depends on voting mechanism
    timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_abstain BOOLEAN DEFAULT FALSE,
    tokens_invested INTEGER, -- For weighted/campaign voting
    UNIQUE (proposal_id, user_id), -- Ensures one vote per user per proposal
    FOREIGN KEY (proposal_id) REFERENCES proposals(proposal_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS campaigns (
    ...
    status TEXT NOT NULL CHECK(status IN ('pending_approval', 'setup', 'active', 'completed', 'archived', 'rejected')) DEFAULT 'pending_approval',
...
CREATE TABLE IF NOT EXISTS proposals (
    ...
    server_id INTEGER NOT NULL,
    proposer_id INTEGER NOT NULL, -- User ID of the proposer
    title TEXT NOT NULL,
    description TEXT,
    voting_mechanism TEXT NOT NULL,
    status TEXT DEFAULT 'Pending' CHECK(status IN ('Pending', 'Voting', 'Closed', 'Cancelled', 'Pending Approval', 'Rejected', 'ApprovedScenario')), -- Added 'ApprovedScenario'
    deadline TEXT, -- ISO format timestamp
    requires_approval BOOLEAN DEFAULT TRUE, -- ADDED THIS LINE
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    results_pending_announcement BOOLEAN DEFAULT FALSE, -- New column
    approved_by INTEGER, -- User ID of admin who approved, if applicable
    rejection_reason TEXT, -- Reason for rejection, if applicable
    hyperparameters TEXT, -- JSON string for mechanism-specific settings
    campaign_id INTEGER, -- Foreign key to campaigns table
    scenario_order INTEGER, -- Order of this proposal if it's a scenario in a campaign
    results_message_id INTEGER, -- Added from previous version of schema, ensure it's here
    results_channel_id INTEGER, -- Added from previous version of schema, ensure it's here
    vote_tracking_message_id INTEGER, -- Added from previous version of schema, ensure it's here
    FOREIGN KEY (server_id) REFERENCES servers(server_id),
    FOREIGN KEY (proposer_id) REFERENCES users(user_id), -- Assuming a users table
    FOREIGN KEY (approved_by) REFERENCES users(user_id), -- Assuming a users table
    FOREIGN KEY (campaign_id) REFERENCES campaigns(campaign_id) ON DELETE SET NULL -- Allow campaign deletion without deleting scenarios
);
"""

CREATE_PROPOSAL_RESULTS_TABLE = """
CREATE TABLE IF NOT EXISTS proposal_results (
    result_id INTEGER PRIMARY KEY AUTOINCREMENT,
    proposal_id INTEGER NOT NULL UNIQUE,
    results TEXT NOT NULL, -- JSON string holding the outcome, winner(s), percentages, etc.
...
CREATE TABLE IF NOT EXISTS warnings (
    ...
    timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    level INTEGER, -- Optional: if warnings escalate
    FOREIGN KEY (server_id) REFERENCES servers(server_id),
    FOREIGN KEY (user_id) REFERENCES users(user_id),
...
CREATE TABLE IF NOT EXISTS temp_moderation (
    ...
    user_id INTEGER NOT NULL,
    moderator_id INTEGER NOT NULL,
    action_type TEXT NOT NULL, -- e.g., 'mute', 'temp_ban'
    reason TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE_VOTING_INVITES_TABLE = """
CREATE TABLE IF NOT EXISTS voting_invites (
    invite_id INTEGER PRIMARY KEY AUTOINCREMENT,
    proposal_id INTEGER NOT NULL,
    voter_id INTEGER NOT NULL,
    status TEXT DEFAULT 'pending', -- e.g., pending, sent, failed, viewed
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
async def add_server(server_id, server_name, owner_id, member_count):
    ...
    async with get_db() as conn:
        await conn.execute(
            "INSERT INTO servers (server_id, server_name, owner_id, member_count, created_at) VALUES (?, ?, ?, ?, ?) "
            "ON CONFLICT(server_id) DO NOTHING",
            (server_id, server_name, owner_id, member_count, current_time_iso)
        )
        await conn.commit()
...
async def update_setting(server_id, setting_key, setting_value):
    async with get_db() as conn:
        await conn.execute(
            ...
            (server_id, setting_key, setting_value) # setting_key and setting_value are parameters, they map to 'key' and 'value' columns
        )
        await conn.commit()
...
async def get_settings(server_id):
    async with get_db() as conn:
        async with conn.execute("SELECT key, value FROM settings WHERE server_id = ?", (server_id,)) as cursor:
    ...
    async with get_db() as conn:
        for var_name, var_data in defaults.items():
            await conn.execute(
                ...
                (server_id, name, value, type, description, last_updated_timestamp)
                VALUES (?, ?, ?, ?, ?, ?)
                ON CONFLICT(server_id, name) DO NOTHING
                """,
                (server_id, var_name, var_data["value"], var_data["type"], var_data["description"], current_time_iso) # Add current_time_iso
            )
        await conn.commit()
...
async def get_constitutional_variable(server_id, variable_name):
    ...
    async with get_db() as conn:
        async with conn.execute(
            "SELECT value, type FROM constitutional_variables WHERE server_id = ? AND name = ?",
...
async def create_proposal(
    ...
    initial_status: Optional[str] = None # New parameter
):
    ...
    # proposal_text_to_store = description_to_store # Removed as proposal_text column doesn't exist
    ...
    # guild_id_to_store = server_id # This line is removed as guild_id column doesn't exist in proposals table

    sql = """
        INSERT INTO proposals
        (server_id, proposer_id, title, description, -- Removed proposal_text
         voting_mechanism, deadline, requires_approval, status, hyperparameters,
         created_at, updated_at, campaign_id, scenario_order)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    params = (
        server_id, proposer_id, title, description_to_store, # Removed proposal_text_to_store
...
async def get_proposal(proposal_id):
    ...
    async with get_db() as conn:
        async with conn.execute(
            "SELECT * FROM proposals WHERE proposal_id = ?", # Removed OR id = ?
            (proposal_id,) # Only one parameter needed now
        ) as cursor:
            row = await cursor.fetchone()
            if row:
                column_names = [desc[0] for desc in cursor.description]
                proposal = {column_names[i]: row[i] for i in range(len(row))}
                # Removed fallback: if proposal.get('proposal_id') is None and 'id' in proposal:
...
async def update_proposal_status(proposal_id, new_status, approved_by=None):
    ...
    async with get_db() as conn:
        if approved_by:
            await conn.execute(
                "UPDATE proposals SET status = ?, approved_by = ?, updated_at = CURRENT_TIMESTAMP WHERE proposal_id = ?",
                (new_status, approved_by, proposal_id),
            )
        else:
            await conn.execute(
                "UPDATE proposals SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE proposal_id = ?",
...
async def store_proposal_results(proposal_id, results_dict):
    ...
    async with get_db() as conn:
        # First check if the proposal_id is valid
        async with conn.execute(
            "SELECT proposal_id FROM proposals WHERE proposal_id = ?", # Simplified query
            (proposal_id,)
        ) as cursor:
            row = await cursor.fetchone()
            if row:
                actual_id = row[0] # Use the fetched proposal_id
...
async def get_proposal_results(proposal_id):
    ...
    async with get_db() as conn:
        async with conn.execute(
            "SELECT results FROM proposal_results WHERE proposal_id = ?", # Simplified query
            (proposal_id,)
        ) as cursor:
            row = await cursor.fetchone()
            if row:
...
async def update_proposal(proposal_id, update_data):
    ...
    async with get_db() as conn:
        ...
        # Removed fallback block that tried to update using 'id' column
        ...
        # Verify the update
        async with conn.execute(
            "SELECT proposal_id, status, results_pending_announcement FROM proposals WHERE proposal_id = ?", # Simplified query
            (proposal_id,)
        ) as cursor:
            row = await cursor.fetchone()
            if row:
                print(f"DEBUG: After update - Proposal {row[0]}: status={row[1]}, results_pending_announcement={row[2]}")
            else:
...
async def _try_drop_column(conn: aiosqlite.Connection, table_name: str, column_name: str):
    """Attempts to drop a column from a table if it exists.
    Prints a warning if dropping fails (e.g., older SQLite version)."""
    try:
        ...
        if column_name in columns:
            print(f"Attempting to drop column '{column_name}' from table '{table_name}'...")
            await conn.execute(f"ALTER TABLE {table_name} DROP COLUMN {column_name};")
            await conn.commit() # Commit after successful drop
            print(f"Successfully dropped column '{column_name}' from table '{table_name}'.")
        # else:
            # print(f"Column '{column_name}' not found in table '{table_name}', no action needed to drop.")

    except sqlite3.OperationalError as e:
        if "no such column" in str(e).lower() or "has no column named" in str(e).lower():
             # This can happen if PRAGMA table_info is slightly off or if the column was already dropped
             print(f"Info: Column '{column_name}' likely already removed or never existed in '{table_name}'. Error: {e}")
        elif "near \"DROP\": syntax error" in str(e).lower() or "cannot drop column" in str(e).lower(): # Older SQLite
            print(f"WARNING: SQLite version may not support DROP COLUMN. Could not drop '{column_name}' from '{table_name}'. Error: {e}")
        else:
            print(f"ERROR trying to drop column '{column_name}' from '{table_name}': {e}")
            # Optionally re-raise if it's an unexpected error: raise
    except Exception as ex: # Catch any other unexpected errors
        print(f"Unexpected ERROR trying to drop column '{column_name}' from '{table_name}': {ex}")
...
async def init_db() -> None:
    ...
    db_operations = [
        PRAGMA_WAL, # Enable WAL mode first
        CREATE_SERVERS_TABLE,
        CREATE_USERS_TABLE, # Ensure users table is created before tables that reference it
        CREATE_SETTINGS_TABLE,
        CREATE_CONSTITUTIONAL_VARIABLES_TABLE,
        ...
        CREATE_CAMPAIGNS_TABLE, # Added Campaign table
        CREATE_USER_CAMPAIGN_PARTICIPATION_TABLE, # Added User Campaign Participation table
        CREATE_VOTING_INVITES_TABLE, # Added
        # Add other CREATE TABLE statements here in dependency order
    ]
    ...
    async with get_db() as conn:
        ...
        # Attempt to drop old problematic columns from 'proposals' table first
        await _try_drop_column(conn, "proposals", "creation_timestamp")
        await _try_drop_column(conn, "proposals", "last_updated_timestamp")
        # Ensure specific columns that might have been missed by CREATE IF NOT EXISTS are present
        await _ensure_column(conn, "proposals", "results_pending_announcement BOOLEAN DEFAULT FALSE")
        await _ensure_column(conn, "proposals", "created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
        await _ensure_column(conn, "proposals", "updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
        # Add other critical _ensure_column calls here if needed for other tables/columns
...
async def get_proposals_with_pending_announcements():
    ...
    async with get_db() as conn:
        ...
        async with conn.execute(
            """
            SELECT proposal_id, status, results_pending_announcement
            FROM proposals
            ...
            for row in debug_rows:
                proposal_identifier = row[0] # proposal_id is now the first column
                # print(f"DEBUG: Proposal {proposal_identifier}: status={row[1]}, results_pending_announcement={row[2]}")
        ...
        async with conn.execute(
            ...
            rows = await cursor.fetchall()
            if rows:
                column_names_all = [desc[0] for desc in cursor.description] # Renamed to avoid conflict
                proposals = [{column_names_all[i]: row[i] for i in range(len(row))} for row in rows]
                print(f"DEBUG: Found {len(proposals)} proposals with pending announcements")

                # Ensure each proposal has a proposal_id (should be guaranteed by SELECT *)
                # for proposal in proposals:
                #     if proposal.get('proposal_id') is None: # Simplified check
                #         print(f"WARN: Proposal missing proposal_id key in get_proposals_with_pending_announcements. Data: {proposal}")
                return proposals
            ...
            async with conn.execute(
                ...
                if fallback_rows:
                    column_names_fallback = [desc[0] for desc in fallback_cursor.description] # Renamed
                    proposals = [{column_names_fallback[i]: row[i] for i in range(len(row))} for row in fallback_rows]
                    print(f"DEBUG: Found {len(proposals)} proposals with pending announcements using fallback query")

                    # Ensure each proposal has a proposal_id (should be guaranteed by SELECT *)
                    # for proposal in proposals:
                    #     if proposal.get('proposal_id') is None: # Simplified check
                    #          print(f"WARN: Proposal missing proposal_id key in get_proposals_with_pending_announcements (fallback). Data: {proposal}")
                    return proposals
    ...
    try:
        async with get_db() as db:
            cursor = await db.execute(
                "INSERT INTO campaigns (guild_id, creator_id, title, description, total_tokens_per_voter, num_expected_scenarios, creation_timestamp, status, current_defined_scenarios) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending_approval', 0)",
                (guild_id, creator_id, title, description, total_tokens_per_voter, num_expected_scenarios, current_time_utc)
            )
            await db.commit()
            campaign_id = cursor.lastrowid
            print(f"DEBUG: Campaign created with ID {campaign_id}, status 'pending_approval'")
            return campaign_id
    except Exception as e:
...
async def get_campaigns_by_status(guild_id: int, status: str) -> List[Dict[str, Any]]:
    """Retrieve all campaigns for a given guild with a specific status."""
    async with get_db() as conn:
        cursor = await conn.execute(
            "SELECT * FROM campaigns WHERE guild_id = ? AND status = ?", (guild_id, status)
        )
        rows = await cursor.fetchall()
        return [dict(row) for row in rows]

async def get_proposals_by_campaign_id(campaign_id: int, guild_id: Optional[int] = None) -> List[Dict[str, Any]]:
    """Fetch all proposals associated with a specific campaign ID."""
    async with get_db() as conn:
        # Filter by guild_id if provided, otherwise just by campaign_id
        sql = "SELECT * FROM proposals WHERE campaign_id = ?"
        params: tuple = (campaign_id,)
        if guild_id is not None:
            sql += " AND server_id = ?" # Assuming server_id is the correct column for guild context
            params = (campaign_id, guild_id)
        ...
        async with conn.execute(sql, params) as cursor:
            ...
            proposals_list = []
            for row in rows:
                proposal = dict(zip(column_names, row))
                # Ensure 'proposal_id' key exists, handling potential 'id' column
                if 'proposal_id' not in proposal and 'id' in proposal:
                    proposal['proposal_id'] = proposal['id']
                ...
                if hp_json:
                    try:
                        ...
                        print(f"Warning: Invalid JSON in hyperparameters for proposal {proposal.get('proposal_id', proposal.get('id'))}. Defaulting to empty dict. JSON: {hp_json}")
                        proposal['hyperparameters'] = {}
                else:
                    proposal['hyperparameters'] = {}

                # Ensure 'proposal_id' is the definitive key, even if 'id' was also fetched.
                # This helps standardize the proposal dictionary structure.
                if 'id' in proposal and proposal['id'] != proposal.get('proposal_id'):
                    # If 'proposal_id' wasn't set from 'id' earlier (e.g. 'proposal_id' was already present but different, or both null initially)
                    # and 'id' holds a value, ensure 'proposal_id' gets it if it's still None or different.
                    # This case is less likely if the earlier 'id' to 'proposal_id' mapping worked.
                    if proposal.get('proposal_id') is None:
                         proposal['proposal_id'] = proposal['id']

                # Clean up by removing 'id' if it's redundant after ensuring 'proposal_id' is set.
                # But only if 'id' is not the *only* key (which is unlikely given prior checks).
                if 'id' in proposal and 'proposal_id' in proposal and proposal['id'] == proposal['proposal_id'] and 'id' != 'proposal_id': # 'id' != 'proposal_id' check is mostly for future proofing if column names change
                    pass # In this case, 'id' is an alias or old name, 'proposal_id' is preferred.
                         # No deletion needed if they are same key due to `dict(zip(column_names, row))` if a column is aliased as `proposal_id`
                         # If `id` is a distinct column name from `proposal_id` but they hold same value after assignment, then we might consider `del proposal['id']`
                         # However, for now, safer to leave it if it's just an alias from SQL (e.g. `SELECT id as proposal_id ...`)
                         # The critical part is that `proposal['hyperparameters']` is a dict and `proposal['proposal_id']` exists.
...
async def approve_campaign(campaign_id: int, admin_user_id: int) -> bool:
    """Approves a campaign, setting its status to 'setup'."""
    current_time_utc = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')
    try:
        async with get_db() as db:
            cursor = await db.execute(
                "UPDATE campaigns SET status = 'setup', approved_by = ?, approval_timestamp = ? WHERE campaign_id = ? AND status = 'pending_approval'",
                (admin_user_id, current_time_utc, campaign_id)
            )
            await db.commit()
            if cursor.rowcount > 0:
                print(f"DEBUG: Campaign C#{campaign_id} approved by U#{admin_user_id}, status set to 'setup'.")
                return True
            else:
                # Either campaign not found, not in pending_approval, or already actioned.
                async with db.execute("SELECT status FROM campaigns WHERE campaign_id = ?", (campaign_id,)) as c_status:
                    current_status_row = await c_status.fetchone()
                    current_status = current_status_row['status'] if current_status_row else 'Not Found'
                print(f"WARN: Campaign C#{campaign_id} not approved. Current status: {current_status}. Expected 'pending_approval'. (User: {admin_user_id})")
                return False
    except Exception as e:
        print(f"ERROR: Approving campaign C#{campaign_id}: {e}")
        return False

async def reject_campaign(campaign_id: int, admin_user_id: int, reason: str) -> bool:
    """Rejects a campaign, setting its status to 'rejected'."""
    current_time_utc = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')
    try:
        async with get_db() as db:
            cursor = await db.execute(
                "UPDATE campaigns SET status = 'rejected', rejected_by = ?, rejection_timestamp = ?, rejection_reason = ? WHERE campaign_id = ? AND status = 'pending_approval'",
                (admin_user_id, current_time_utc, reason, campaign_id)
            )
            await db.commit()
            if cursor.rowcount > 0:
                print(f"DEBUG: Campaign C#{campaign_id} rejected by U#{admin_user_id}. Reason: {reason}")
                return True
            else:
                async with db.execute("SELECT status FROM campaigns WHERE campaign_id = ?", (campaign_id,)) as c_status:
                    current_status_row = await c_status.fetchone()
                    current_status = current_status_row['status'] if current_status_row else 'Not Found'
                print(f"WARN: Campaign C#{campaign_id} not rejected. Current status: {current_status}. Expected 'pending_approval'. (User: {admin_user_id})")
                return False
    except Exception as e:
        print(f"ERROR: Rejecting campaign C#{campaign_id}: {e}")
        return False

async def set_campaign_control_message_id(campaign_id: int, message_id: int) -> bool:
    """Sets the control_message_id for a campaign."""
    try:
        async with get_db() as db_conn: # Renamed to db_conn to avoid conflict
            await db_conn.execute(
                "UPDATE campaigns SET control_message_id = ? WHERE campaign_id = ?",
                (message_id, campaign_id)
            )
            await db_conn.commit()
            print(f"DEBUG: Set control_message_id {message_id} for C#{campaign_id}")
            return True
    except Exception as e:
        print(f"ERROR: Setting control_message_id for C#{campaign_id}: {e}")
        return False
