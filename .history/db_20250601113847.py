    ...
    async with get_db() as conn:
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS settings (
                ...
                PRIMARY KEY (setting_key, server_id)
            )
        """)
...
async def get_campaign(campaign_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        async with get_db() as conn:
            ...
            async with conn.execute("SELECT * FROM campaigns WHERE campaign_id = ?", (campaign_id,)) as cursor:
...
async def update_campaign_status(campaign_id: int, status: str) -> bool:
    ...
    try:
        async with get_db() as conn:
            await conn.execute("UPDATE campaigns SET status = ? WHERE campaign_id = ?", (status, campaign_id))
            await conn.commit()
...
async def increment_defined_scenarios(campaign_id: int) -> Optional[int]:
    ...
    try:
        async with get_db() as conn:
            await conn.execute("UPDATE campaigns SET current_defined_scenarios = current_defined_scenarios + 1 WHERE campaign_id = ?", (campaign_id,))
            await conn.commit()
            async with conn.execute("SELECT current_defined_scenarios FROM campaigns WHERE campaign_id = ?", (campaign_id,)) as cursor:
...
async def get_campaigns_by_status(guild_id: int, status: str) -> List[Dict[str, Any]]:
    ...
    try:
        async with get_db() as conn:
            db.row_factory = aiosqlite.Row
            async with conn.execute("SELECT * FROM campaigns WHERE guild_id = ? AND status = ?", (guild_id, status)) as cursor:
...
CREATE TABLE IF NOT EXISTS votes (
    ...
    proposal_id INTEGER NOT NULL,
    user_id INTEGER NOT NULL,
    vote_data TEXT NOT NULL, -- For plurality/approval, this is the option_id or comma-separated option_ids. For Borda, JSON array of option_ids in order.
