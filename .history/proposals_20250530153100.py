...
class BaseProposalModal(discord.ui.Modal):
    ...
    async def common_on_submit(self, interaction: discord.Interaction, specific_hyperparameters: Dict[str, Any]):
        ...
        try:
            ...
            allow_abstain = True # Default to true
            if allow_abstain_str == 'no':
                allow_abstain = False
            elif allow_abstain_str == 'yes' or allow_abstain_str == '': # 'yes' or blank explicitly means yes
                allow_abstain = True
            else: # Invalid input
                 await interaction.response.send_message("Invalid input for 'Allow Abstain Votes?'. Must be 'yes', 'no', or blank for yes.", ephemeral=True)
                 return
            ...
            final_hyperparameters = {"allow_abstain": allow_abstain, **specific_hyperparameters}
...
class PluralityProposalModal(BaseProposalModal):
    def __init__(self, interaction: discord.Interaction, mechanism_name: str):
        ...
        # allow_abstain_input is now in BaseProposalModal
        ...
        self.winning_threshold_percentage_input = discord.ui.TextInput(
            ...
            row=0 # This will be the first item specific to this modal, technically row 5 overall if base takes 0-4
        )
        self.add_item(self.winning_threshold_percentage_input)

    async def on_submit(self, interaction: discord.Interaction):
        # allow_abstain is handled by BaseProposalModal.common_on_submit
...
class ApprovalProposalModal(BaseProposalModal):
    def __init__(self, interaction: discord.Interaction, mechanism_name: str):
        ...
        # allow_abstain_input is now in BaseProposalModal
        # No Approval-specific inputs needed in this modal for now.
...
class RunoffProposalModal(BaseProposalModal):
    def __init__(self, interaction: discord.Interaction, mechanism_name: str):
        ...
        # allow_abstain_input is now in BaseProposalModal
        # No Runoff-specific inputs needed in this modal for now.
...
class DHondtProposalModal(BaseProposalModal):
    def __init__(self, interaction: discord.Interaction, mechanism_name: str):
        ...
        self.num_seats_input = discord.ui.TextInput(
            ...
            default="1",
            required=False,
            row=0 # This will be the first item specific to this modal
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            num_seats = int(num_seats_val) if num_seats_val.strip() else 1
            if num_seats < 1:
                await interaction.response.send_message("Number of seats must be 1 or greater.", ephemeral=True)
                return
        except ValueError:
