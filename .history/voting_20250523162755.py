...
# --- Updated send_voting_dm function ---


async def send_voting_dm(member: discord.Member, proposal: Dict, options: Optional[List[str]] = None) -> bool:
    """Send a DM to a member with interactive voting options"""
    if member.bot:
        print(
            f"DEBUG: Skipping sending DM to bot user {member.name} (ID: {member.id})")
        return False

    proposal_id = proposal.get('proposal_id') or proposal.get('id')
    mechanism_name = proposal.get('voting_mechanism', 'Unknown').lower()
    mechanism = voting_utils.get_voting_mechanism(mechanism_name)

    if not mechanism:
        print(
            f"ERROR: Unknown voting mechanism {mechanism_name} for proposal {proposal_id}")
        return False

    if options is None:
        # Get options from DB, fallback to description/YesNo
        options = await db.get_proposal_options(proposal_id) or utils.extract_options_from_description(proposal.get('description', '')) or ["Yes", "No"]

    embed = discord.Embed(
        title=f"Proposal #{proposal_id}: {proposal.get('title', 'Untitled')}",
        description=proposal.get('description', ''),
        color=discord.Color.blue()
    )

    embed.add_field(name="Voting Mechanism",
                    value=f"{mechanism_name.title()}", inline=False)
    if hasattr(mechanism, 'get_description'):
        embed.add_field(name="Mechanism Description",
                        value=mechanism.get_description(), inline=False)

    deadline_str_formatted = utils.format_deadline(proposal.get('deadline'))
    if deadline_str_formatted != "Not Set":
        embed.add_field(name="Deadline",
                        value=deadline_str_formatted, inline=False)
    ...
    view = None
    # Check if options list is not empty before creating views that require options
    if options:
        if mechanism_name in ["plurality", "dhondt"]:
            view = PluralityVoteView(proposal_id, options, member.id, True)
        elif mechanism_name in ["borda", "runoff"]:
            if len(options) > 1:  # Ranked voting needs at least 2 options
                view = RankedVoteView(proposal_id, options, member.id, True)
            else:
                print(
                    f"WARNING: Not enough options ({len(options)}) for Ranked voting view for proposal {proposal_id}")
        elif mechanism_name == "approval":
            view = ApprovalVoteView(proposal_id, options, member.id, True)
        else:
            print(
                f"WARNING: Interactive view not available for mechanism: {mechanism_name}")
    else:
        print(
            f"WARNING: No options available for proposal {proposal_id}. Skipping interactive view.")
    ...
    try:
        ...
        if view:
            print(
                f"DEBUG: Sending interactive DM to {member.name} (ID: {member.id}) for proposal {proposal_id}")
            await dm_channel.send(
                content="You're invited to vote on a new proposal. Use the interactive options below to cast your vote:",
                embed=embed,
                view=view
            )

            await dm_channel.send(
                "If the interactive interface doesn't work or you prefer, you can still vote using text commands in this DM channel:\n"
                f"• Plurality/D'Hondt: `!vote {proposal_id} <option>`\n"
                f"• Borda/Runoff: `!vote {proposal_id} rank option1,option2,...`\n"
                f"• Approval: `!vote {proposal_id} approve option1,option2,...`\n"
                f"• Abstain: `!vote {proposal_id} abstain`\n"
                "*(Replace `<option>`, `option1,option2,...` with your actual choices. Quote options with spaces.)*"
            )

        else:
            print(
                f"DEBUG: Falling back to text-based DM for proposal {proposal_id}")
            await dm_channel.send(content="You're invited to vote on a new proposal:", embed=embed)

            instructions = "Instructions not available for this voting mechanism."
            if hasattr(mechanism, 'get_vote_instructions'):
                instructions = mechanism.get_vote_instructions()
            await dm_channel.send(f"**How to vote:**\n{instructions}\n\nTo abstain, send: `!vote {proposal_id} abstain`")

            example = ""
            if options:  # Provide example only if options exist
                if mechanism_name == 'plurality' or mechanism_name == 'dhondt':
                    example = f"Example: `!vote {proposal_id} \"{options[0]}\"`"
                elif mechanism_name == 'borda' or mechanism_name == 'runoff':
                    if len(options) >= 2:
                        example = f"Example: `!vote {proposal_id} rank \"{options[0]}\",\"{options[1]}\"`"
                    elif options:
                        example = f"Example: `!vote {proposal_id} rank \"{options[0]}\"`"
                elif mechanism_name == 'approval':
                    if len(options) >= 2:
                        example = f"Example: `!vote {proposal_id} approve \"{options[0]}\",\"{options[1]}\"`"
                    elif options:
                        example = f"Example: `!vote {proposal_id} approve \"{options[0]}\"`"
        ...
        print(
            f"WARNING: Could not send voting DM to user {member.name} (ID: {member.id}) - DMs disabled?")
        return False
    except Exception as e:
        print(
            f"ERROR sending voting DM to user {member.name} (ID: {member.id}) for proposal {proposal_id}: {e}")
        traceback.print_exc()
        return False


# --- process_vote function code ---
# This function's timestamp handling and validation were updated in the previous turn.
# Keep the code as provided in the previous answer.
