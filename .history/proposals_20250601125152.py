    ...
    try:
        ...
        if campaign_id:
            ...
            if campaign and campaign['status'] in ['setup', 'active']:
                print(f"DEBUG: P#{proposal_id_for_debug} (Scenario for C#{campaign_id}) - Campaign is approved (status: {campaign['status']}). Setting requires_approval=False for this scenario.")
                requires_approval = False
            elif campaign:
                # Campaign exists but is not in a state where scenarios are auto-approved (e.g., pending_approval itself)
                print(f"WARN: P#{proposal_id_for_debug} (Scenario for C#{campaign_id}) - Campaign status is '{campaign['status']}'. This scenario will follow default approval rules (likely requiring approval).")
                # requires_approval will be determined by const_vars below if not set to False above
            else:
                # Campaign not found, which is unexpected if campaign_id is provided.
                print(f"ERROR: P#{proposal_id_for_debug} (Scenario for C#{campaign_id}) - Campaign not found. This scenario will follow default approval rules.")
                # requires_approval will be determined by const_vars below

        # If requires_approval is still True (i.e., not a scenario of an approved campaign),
        ...
        if requires_approval:
            try:
                ...
                print(f"Notice: Could not fetch constitutional_vars for proposal_requires_approval, defaulting to True (if not already set by campaign logic). Error: {e_cv}")
