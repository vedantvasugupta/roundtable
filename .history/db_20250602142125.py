...
async def init_db() -> None:
    ...
    async with get_db() as conn:
        ...
        # Drop settings table to ensure schema update for UNIQUE constraint (dev only)
        await conn.execute("DROP TABLE IF EXISTS settings")
        print("INFO: Dropped 'settings' table if it existed to ensure schema update.")

        await conn.execute("""
            CREATE TABLE IF NOT EXISTS settings (
                setting_id INTEGER PRIMARY KEY AUTOINCREMENT,
                server_id INTEGER NOT NULL, -- Removed UNIQUE from here
                key TEXT NOT NULL,
                value TEXT NOT NULL,
                FOREIGN KEY (server_id) REFERENCES servers(server_id) ON DELETE CASCADE,
                UNIQUE (server_id, key) -- Added composite UNIQUE constraint here
            )
        """)

        await conn.execute(CREATE_CAMPAIGNS_TABLE) # Defined as a global constant with the NEW schema

        # --- Schema migration for campaigns.status CHECK constraint ---
        try:
            async with conn.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='campaigns';") as cursor:
                row = await cursor.fetchone()
                if row and row["sql"]:
                    current_schema_sql = row["sql"]
                    # Check if the constraint is outdated (doesn't mention pending_approval)
                    # A more robust check would be to parse constraints properly, but string check is simpler for this specific case.
                    old_constraint_fragment = "status IN ('setup', 'active', 'completed', 'archived')"
                    new_constraint_fragment = "pending_approval"
                    if old_constraint_fragment in current_schema_sql and new_constraint_fragment not in current_schema_sql:
                        print("INFO: Outdated 'campaigns' table schema detected for status CHECK constraint. Attempting migration...")
                        await conn.execute("BEGIN TRANSACTION;")
                        temp_table_name = "campaigns_old_migration_" + datetime.utcnow().strftime("%Y%m%d%H%M%S")
                        await conn.execute(f"ALTER TABLE campaigns RENAME TO {temp_table_name};")
                        print(f"INFO: Renamed existing 'campaigns' table to '{temp_table_name}'.")

                        await conn.execute(CREATE_CAMPAIGNS_TABLE) # Create new table with correct schema
                        print("INFO: Created new 'campaigns' table with updated schema.")

                        # Get columns from old and new tables to copy intersecting data
                        async with conn.execute(f"PRAGMA table_info({temp_table_name})") as old_cols_cursor:
                            old_columns = [col[1] for col in await old_cols_cursor.fetchall()]
                        async with conn.execute("PRAGMA table_info(campaigns)") as new_cols_cursor:
                            new_columns = [col[1] for col in await new_cols_cursor.fetchall()]

                        common_columns = [col for col in old_columns if col in new_columns]
                        common_columns_str = ", ".join(common_columns)

                        await conn.execute(f"INSERT INTO campaigns ({common_columns_str}) SELECT {common_columns_str} FROM {temp_table_name};")
                        print(f"INFO: Copied data from '{temp_table_name}' to new 'campaigns' table.")

                        await conn.execute(f"DROP TABLE {temp_table_name};")
                        print(f"INFO: Dropped temporary table '{temp_table_name}'.")
                        await conn.execute("COMMIT;")
                        print("INFO: 'campaigns' table schema migration completed successfully.")
                    else:
                        print("INFO: 'campaigns' table schema for status CHECK constraint appears up-to-date or table was just created.")
                else:
                    print("INFO: 'campaigns' table not found or SQL schema could not be retrieved (likely just created by CREATE_CAMPAIGNS_TABLE above).")
        except Exception as e_migration:
            await conn.execute("ROLLBACK;") # Rollback on error during migration
            print(f"ERROR: During 'campaigns' table schema migration: {e_migration}. Rolled back transaction.")
            # Potentially re-raise or handle more gracefully depending on how critical this is for startup
        ...
        # Drop constitutional_variables table to ensure schema update (dev only)
        await conn.execute("DROP TABLE IF EXISTS constitutional_variables")
        print("INFO: Dropped 'constitutional_variables' table if it existed to ensure schema update.")
        ...
        await _ensure_column(conn, 'campaigns', 'control_message_id INTEGER') # New ensure_column call
