...
class CampaignSetupModal(discord.ui.Modal, title="Create New Weighted Campaign"):
    def __init__(self, original_interaction: discord.Interaction):
        ...
        self.campaign_description_input = discord.ui.TextInput(
            ...
            max_length=1000, # Discord text input limit for paragraph
            required=False,
        ...
        self.total_tokens_input = discord.ui.TextInput(
            label="Total Tokens Per Voter (for the entire campaign)",
            placeholder="e.g., 100",
            min_length=1,
            max_length=5,
            required=True,
        ...
        self.num_scenarios_input = discord.ui.TextInput(
            label="Number of Voting Scenarios/Rounds in this Campaign",
            placeholder="e.g., 3",
            min_length=1,
            max_length=2,
            required=True,
        ...
        try:
            ...
            try:
                ...
                if num_scenarios <= 0:
                    ...
                    return
                if num_scenarios > 10: # Practical limit for now
            ...
            if campaign_id:
                view = DefineScenarioView(campaign_id=campaign_id, next_scenario_order=1, total_scenarios=num_scenarios, original_interaction=self.original_interaction)
                await interaction.followup.send(
                    ...
                    view=view,
                    ephemeral=False # Make this visible so creator can proceed
                )
                # Try to edit the original interaction that opened the mechanism selection, if it was from there.
                if self.original_interaction and not self.original_interaction.is_expired():
                    try:
                        ...
                        pass # It might have been a different kind of interaction or already handled
            else:
...
class DefineScenarioView(discord.ui.View):
    def __init__(self, campaign_id: int, next_scenario_order: int, total_scenarios: int, original_interaction: Optional[discord.Interaction] = None):
        super().__init__(timeout=300) # 5 min timeout for defining next step
        self.campaign_id = campaign_id
        self.next_scenario_order = next_scenario_order
        self.total_scenarios = total_scenarios
        self.original_interaction = original_interaction # The interaction that submitted the CampaignSetupModal or previous DefineScenarioView button
    ...
    async def define_scenario_callback(self, interaction: discord.Interaction):
        # Present the ProposalMechanismSelectionView, but now pass campaign context
        # The original_interaction for ProposalMechanismSelectionView will be this current button interaction
        view = ProposalMechanismSelectionView(
            original_interaction=interaction,
            invoker_id=interaction.user.id,
            campaign_id=self.campaign_id, # Pass campaign context
            scenario_order=self.next_scenario_order # Pass scenario order
        )
        await interaction.response.send_message(
            ...
            view=view,
            ephemeral=True # Mechanism selection itself can be ephemeral, modal will be new interaction
        )
        # Disable this button after click
        self.define_button.disabled = True
        if self.message: # If we have the message this view is attached to
            await self.message.edit(view=self)
        self.stop() # Stop this instance of DefineScenarioView
...
class ProposalMechanismSelectionView(discord.ui.View):
    ...
    def __init__(self, original_interaction: Optional[discord.Interaction], invoker_id: int, campaign_id: Optional[int] = None, scenario_order: Optional[int] = None):
        ...
        if not self.campaign_id:
            wc_button = discord.ui.Button(
                ...
                style=discord.ButtonStyle.success, # Differentiate it
                custom_id="select_mechanism_weighted_campaign",
                emoji="⚖️💰",
                row = (len(mechanisms) // 2) +1 # Place on a new row
            )
    ...
    async def weighted_campaign_button_callback(self, interaction: discord.Interaction):
        ...
        modal = CampaignSetupModal(original_interaction=self.original_interaction)
        await interaction.response.send_modal(modal)
        self.stop()
        # Edit original message to indicate modal was sent
        if self.original_interaction and not self.original_interaction.is_expired():
            try:
                await self.original_interaction.edit_original_response(content="Weighted Campaign setup form sent. Please fill it out.", view=None)
            except discord.HTTPException:
                pass
