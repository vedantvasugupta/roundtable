...
async def process_vote(user_id: int, proposal_id: int, vote_data: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    try:
        print(
            f"DEBUG: Processing vote for proposal {proposal_id} from user {user_id} with data: {vote_data}")
        ...
        # Deadline check (simplified for brevity, assuming it's handled correctly)
        deadline_data = proposal.get('deadline')
        if deadline_data:
            # Simplified: assume deadline_data is parseable string or datetime
        ...
        # Mechanism-specific validation might occur here or within db.record_vote
        # For now, assume vote_data is structured correctly by the View

        # Key change: db.record_vote needs to handle the full vote_data structure
        # including 'tokens_invested' and 'did_abstain'
        # The db.record_vote function in db.py has been updated for this.

        # Let's assume process_vote correctly calls db.record_vote which handles these new fields.
        # For this edit, we are focusing on the View logic sending the correct data to process_vote.
        # Simplified call for this example, actual process_vote is more complex.

        # The actual recording happens in db.record_vote, called by process_vote
        # We need to make sure process_vote passes all necessary fields from vote_data to db.record_vote

        # Simulate the db.record_vote part that process_vote would call
        # This is a placeholder for the actual logic within process_vote
        record_success = await db.record_vote(
            user_id=user_id,
            proposal_id=proposal_id,
            # vote_data here is the *string* representation for the 'vote_data' column.
            # So, process_vote should ensure it serializes the parts of vote_data that go into that column,
            # and passes 'is_abstain' and 'tokens_invested' as separate parameters to db.record_vote.
            # The current db.record_vote takes: user_id, proposal_id, vote_data (json string), is_abstain, tokens_invested

            vote_data=json.dumps({k: v for k, v in vote_data.items() if k not in ['did_abstain', 'tokens_invested']}), # Main vote payload
            is_abstain=vote_data.get('did_abstain', False),
            tokens_invested=vote_data.get('tokens_invested')
        )

        if record_success:
            message = "Vote recorded."
            # Trigger update_vote_tracking (simplified)
            guild = None
            if proposal.get('server_id'):
                # In a real scenario, you'd get the bot instance to find the guild
                # For this example, we'll assume it might be available.
                pass
            if guild:
                await voting_utils.update_vote_tracking(guild, proposal_id)

            # Simplified 100% vote check
            # ...
            return True, message
        else:
            return False, "Failed to record vote in database."

    except Exception as e:
        print(f"CRITICAL ERROR processing vote (simplified stub) for user {user_id} proposal {proposal_id}: {e}")
        traceback.print_exc()
