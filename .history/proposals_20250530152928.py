...
class BaseProposalModal(discord.ui.Modal):
    ...
    async def common_on_submit(self, interaction: discord.Interaction, specific_hyperparameters: Dict[str, Any]):
        ...
        try:
            ...
            allow_abstain_str = self.allow_abstain_input.value.lower()

            allow_abstain = allow_abstain_str == 'yes'
            if allow_abstain_str not in ['yes', 'no', '']:
                 await interaction.response.send_message("Invalid input for 'Allow Abstain Votes?'. Must be 'yes' or 'no'.", ephemeral=True)
                 return
            if allow_abstain_str == '': # Default to yes if blank
                allow_abstain = True
            ...
            # Merge common hyperparameters with specific ones
            # Specific hyperparameters take precedence if keys overlap
