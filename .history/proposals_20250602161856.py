...
class AdminApprovalView(discord.ui.View):
    def __init__(self, proposal_id: int, bot_instance: commands.Bot, campaign_id: Optional[int] = None, scenario_order: Optional[int] = None):
        ...
        self.campaign_id = campaign_id
        self.scenario_order = scenario_order # Ensure this line is present and correct
    ...
    async def approve_button_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Ensure self.scenario_order is passed here
        success, message_content = await _perform_approve_proposal_action(
            interaction,
            self.proposal_id,
            self.bot,
            campaign_id=self.campaign_id,
            scenario_order=self.scenario_order # Pass the stored scenario_order
        )
        ...
        if success:
            ...
            for item in self.children:
                if isinstance(item, discord.ui.Button) and item.custom_id == "admin_reject_proposal":
                    item.disabled = True
    ...
    async def reject_button_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        modal = RejectReasonModal(
            ...
            parent_view=self,
            campaign_id=self.campaign_id,
            scenario_order=self.scenario_order # Pass scenario_order
        )
        await interaction.response.send_modal(modal)
...
class CampaignControlView(discord.ui.View):
    ...
    async def update_button_states(self, interaction: Optional[discord.Interaction] = None):
        ...
        if campaign['status'] == 'setup':
            ...
            if approved_scenarios_for_start: # Check if list is not empty
                ...
                first_defined_not_approved = any(p.get('status') == 'Pending Approval' for p in proposals_in_campaign if p.get('scenario_order') == 1)
                if first_defined_not_approved:
                    self.start_next_button.label = "Await S1 Approval"
                else: # Defined, but some other status, or S1 approved but others not relevant for *starting* campaign
                    self.start_next_button.label = "Verify S1 Status" # Generic if S1 defined but not clearly pending approval
                self.start_next_button.disabled = True
            else:
            ...
            if active_voting_scenarios:
                ...
                if next_scenario_order_needed > campaign['num_expected_scenarios']:
                    ...
                    # Check if the next scenario is defined AND approved to 'ApprovedScenario'
                    ...
                    if next_scenario_to_start and next_scenario_to_start.get('status') == 'ApprovedScenario':
                        ...
                        self.start_next_button.disabled = False
                    elif next_scenario_to_start: # Defined but not (yet) 'ApprovedScenario'
                        self.start_next_button.label = f"Await S{next_scenario_order_needed} Approval"
                        self.start_next_button.disabled = True
                    else: # Not even defined
    ...
    async def start_next_callback(self, interaction: discord.Interaction):
        # This method is called when the "Start Campaign" button is clicked.
        # You can implement the logic you want to execute when this button is clicked.
        await interaction.response.send_message("Start Campaign button clicked!", ephemeral=True)
