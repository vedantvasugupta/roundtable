...
async def init_db() -> None:
    ...
    async with get_db() as conn:
        ...
        # ... (CREATE TABLE statements - keep them as they are) ...
        # These CREATE TABLE statements correctly use DEFAULT CURRENT_TIMESTAMP
        # for *newly created tables*. The _ensure_column is only for *existing* tables
        # that might be missing columns from a schema update.
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS proposals (
                ...
                scenario_order INTEGER, -- Order of this proposal within a campaign
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS votes (
                ...
                tokens_invested INTEGER, -- Number of tokens invested for campaign votes
                FOREIGN KEY (proposal_id) REFERENCES proposals(proposal_id) ON DELETE CASCADE
        """)
        ...
        # Ensure necessary columns exist in case schema changed
        # These calls use the updated _ensure_column logic
        await _ensure_column(
            conn,
            "proposals",
            "results_pending_announcement INTEGER DEFAULT 0"
        )
        ...
        # ADD THIS CALL for the tracking_message_id column
        await _ensure_column(
            conn,
            "proposals",
            "tracking_message_id TEXT"  # TEXT DEFAULT NULL is implicit
        )
