...
async def test_result_calculation_campaigns():
    ...
    # Close all scenarios properly using close_proposal function which stores results
    for scenario_id in scenarios:
        # Move scenarios to Voting status first
        await db.update_proposal_status(scenario_id, "Voting")
        # Then close them properly which calculates and stores results
        results = await voting_utils.close_proposal(scenario_id)
        assert results is not None, f"Failed to close scenario {scenario_id}"
        print(f"✅ Closed scenario P#{scenario_id}")
