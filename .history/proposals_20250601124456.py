...
class AdminApprovalView(discord.ui.View):
    def __init__(self, proposal_id: int, bot_instance: commands.Bot, campaign_id: Optional[int] = None, scenario_order: Optional[int] = None):
        ...
        self.scenario_order = scenario_order # Store for context
...
class AdminCampaignApprovalView(discord.ui.View):
    ...
    async def approve_campaign_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Placeholder: Defer and log for now
        await interaction.response.defer()
        print(f"Admin {interaction.user.name} clicked Approve for Campaign ID: {self.campaign_id}")
        # TODO: Implement _perform_approve_campaign_action call
        # TODO: Update original message (disable buttons, change embed)
        await interaction.followup.send(f"Campaign {self.campaign_id} approval action (approve) placeholder.", ephemeral=True)
    ...
    async def reject_campaign_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Placeholder: Defer and log for now
        await interaction.response.defer()
        print(f"Admin {interaction.user.name} clicked Reject for Campaign ID: {self.campaign_id}")
        # TODO: Implement modal for rejection reason, then _perform_reject_campaign_action call
        # TODO: Update original message (disable buttons, change embed)
        await interaction.followup.send(f"Campaign {self.campaign_id} rejection action (reject) placeholder.", ephemeral=True)
...
class StartScenarioDefinitionView(discord.ui.View):
    ...
    async def define_scenarios_callback(self, interaction: discord.Interaction):
        ...
        # Create and send the DefineScenarioView to the creator (ephemerally)
        ...
        # Send a new ephemeral message to the creator with the DefineScenarioView
        sent_message = await interaction.followup.send(content=followup_message_content, view=scenario_view, ephemeral=True)
        # scenario_view.message = sent_message # DefineScenarioView might want to edit its own message, ensure it has a way to get it
        # For ephemeral, followup.send doesn't return a message object that can be easily edited by the view in the same way.
        # The DefineScenarioView's original_interaction (this interaction) will be used for its on_timeout to edit the ephemeral msg it sends.

        # Disable this button after use
        self.define_scenarios_button.disabled = True
        self.define_scenarios_button.label = f"Scenario Definition Started"
        try:
            if interaction.message: # Check if message exists (it should, as this view is attached to it)
                 await interaction.message.edit(view=self) # Edit the DM message to disable the button
        except discord.HTTPException as e:
...
async def _perform_approve_campaign_action(admin_interaction_for_message_edit: discord.Interaction, campaign_id: int, bot_instance: commands.Bot, admin_view_to_disable: AdminCampaignApprovalView):
    ...
    if not approved:
        await admin_interaction_for_message_edit.followup.send(f"Failed to approve Campaign C#{campaign_id}. It might have been actioned already or an error occurred.", ephemeral=True)
        return
    ...
    if creator:
        try:
            ...
            await creator.send(
                ...
                f"You can now start defining its scenarios.",
                view=dm_view
            )
        except discord.Forbidden:
    ...
    new_embed.set_footer(text=f"Approved by {admin_user.display_name} at {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')} | Campaign ID: {campaign_id}")
...
async def _perform_reject_campaign_action(admin_interaction_for_message_edit: discord.Interaction, campaign_id: int, reason: str, bot_instance: commands.Bot, admin_view_to_disable: AdminCampaignApprovalView, rejecting_admin_user: discord.User):
    ...
    if not rejected:
        await admin_interaction_for_message_edit.followup.send(f"Failed to reject Campaign C#{campaign_id}. It might have been actioned already or an error occurred.", ephemeral=True)
        # This followup is to the original button interaction, which might be confusing if modal was used.
        # The modal itself got a deferral. Perhaps a followup to modal_submission_interaction if available here.
        return
    ...
    # Followup to the modal submission interaction, if it was passed, to confirm action to admin who submitted modal
    # This function doesn't directly receive modal_submission_interaction. The caller (CampaignRejectReasonModal.on_submit)
    # already deferred its own interaction. This followup is for the button clicker (admin_interaction_for_message_edit).
