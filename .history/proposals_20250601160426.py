...
async def handle_start_campaign_or_next_scenario(interaction: discord.Interaction, campaign_id: int, bot: commands.Bot) -> tuple[bool, str]:
    ...
    if campaign_mgmt_channel and campaign_control_message_id:
        try:
            ...
            print(f"ERROR: Control message {campaign_control_message_id} for C#{campaign_id} not found.")
        except discord.Forbidden:
            print(f"ERROR: Bot lacks permissions for control message {campaign_control_message_id} for C#{campaign_id}.")
    ...
    async def _update_control_message(status_message: str, color: discord.Color = discord.Color.blue()):
        if not original_control_message:
            print(f"WARN: _update_control_message called for C#{campaign_id} but original_control_message is None.")
            return

        # Fetch fresh campaign data for the most up-to-date view state
        ...
        if not current_campaign_data_for_view:
            ...
            # Potentially edit message to show an error state if possible
            error_embed = discord.Embed(title=f"Campaign C#{campaign_id} - Error", description="Could not load campaign data to update control panel.", color=discord.Color.red())
            try: await original_control_message.edit(embed=error_embed, view=None) # Clear buttons on error
            except: pass
            return

        new_view = CampaignControlView(campaign_id, bot) # Create a new view instance
        ...
        try:
            ...
            if proposals_channel:
                ...
                scenario_embed = utils.create_proposal_embed(
                    first_scenario, proposer_member, 'Voting', options, bot, campaign_data=current_campaign_data_after_update
                )
            ...
            if voting_room_channel:
                # Pass the full scenario dict to update_vote_tracking
                await voting_utils.update_vote_tracking(bot, guild, first_scenario)
            ...
            if eligible_voters_list:
                for member_to_dm in eligible_voters_list:
                    if member_to_dm.bot: continue
                    dm_sent = await voting.send_voting_dm(bot, member_to_dm, first_scenario, scenario_options) # Pass bot
    ...
    # --- Start Next Scenario Logic (status 'active') ---
        ...
        if not next_scenario_to_start:
            if campaign['current_defined_scenarios'] < next_scenario_order_to_start <= campaign['num_expected_scenarios']:
                 ...
                 msg = f"Error: Next scenario (order {next_scenario_order_to_start}) not found for C#{campaign_id}. It might not be defined or there's an issue."
        ...
        current_campaign_data_after_update = await db.get_campaign(campaign_id)
        ...
        try:
            ...
            if proposals_channel:
                ...
                scenario_embed = utils.create_proposal_embed(
                    next_scenario_to_start, proposer_member, 'Voting', options, bot, campaign_data=current_campaign_data_after_update
                )
            ...
            voting_room_channel = discord.utils.get(guild.text_channels, name=voting_room_channel_name)
            if voting_room_channel:
                await voting_utils.update_vote_tracking(bot, guild, next_scenario_to_start) # Pass bot and full scenario
            ...
            if eligible_voters_list:
                for member_to_dm in eligible_voters_list:
                    if member_to_dm.bot: continue
                    dm_sent = await voting.send_voting_dm(bot, member_to_dm, next_scenario_to_start, scenario_options) # Pass bot
