...
class ProposalMechanismSelectionView(discord.ui.View):
    ...
    def __init__(self, original_interaction: discord.Interaction):
        ...
        self.invoker_id = original_interaction.user.id
    ...
    async def on_timeout(self):
        for item in self.children:
            item.disabled = True
        try:
            await self.original_interaction.edit_original_response(content="Proposal mechanism selection timed out.", view=self)
        except discord.HTTPException:
            pass  # Original interaction might have already been responded to or timed out
...
async def propose(ctx: commands.Context):
    """Command to propose a new idea using the new mechanism selection view"""
    try:
        print(f"🔍 {ctx.author} invoked !propose in {ctx.guild.name}")
        # Send the new mechanism selection view
        # We need an interaction to initialize ProposalMechanismSelectionView.
        # Since this is a command context, we can't directly get an interaction
        # to pass to the view's __init__ for editing the *original* response later.
        # The view will handle interactions with its own buttons.
        # For the initial message, we just send it.
        # A slightly better UX might be to make /propose a slash command
        # so the initial response is an interaction we can edit.
        # For now, we'll send a new message with the view.
        view = ProposalMechanismSelectionView(ctx.interaction if ctx.interaction else await ctx.send("Please wait...")) # Hacky way to get an interaction, might not always work for ! commands.
                                                                                                   # Better to make /propose a slash command.
                                                                                                   # For now, if ctx.interaction is None, this will fail.
                                                                                                   # Let's assume for now this command will be migrated to a slash command.
                                                                                                   # Or, the view doesn't strictly need the original_interaction if we don't edit the first message.

        # Let's redefine the view init to not require original_interaction,
        # or handle it being None if we can't easily get it from a message command context.
        # For now, let's assume we find a way to pass an interaction.
        # If this is a true hybrid command, ctx.interaction should be populated.

        # Send the view with mechanism selection buttons
        # The ProposalMechanismSelectionView needs the initial interaction that triggered it.
        # If 'ctx' is from a message command, ctx.interaction might be None.
        # This part of the design is tricky for old-style message commands.
        # For robust behavior, /propose should be a slash command.
        # Assuming it IS a slash command or hybrid where ctx.interaction exists:
        if ctx.interaction:
            await ctx.interaction.response.send_message("Please select a voting mechanism for your proposal:", view=ProposalMechanismSelectionView(ctx.interaction), ephemeral=True)
            print(f"✅ ProposalMechanismSelectionView sent to {ctx.channel.name} in {ctx.guild.name} for {ctx.author}")
        else:
            # Fallback for pure message command - this won't be ideal UX for editing the view's message on timeout.
            # The buttons themselves will work.
            initial_msg = await ctx.send("Please select a voting mechanism for your proposal:")
            # Create a dummy interaction-like object for the view if needed, or adapt view
            # For now, this path will have degraded UX for message updates from the view.
            # The view itself uses the new interaction from button clicks.
            await initial_msg.edit(view=ProposalMechanismSelectionView(None)) # Pass None, view needs to handle it.
            print(f"✅ ProposalMechanismSelectionView sent to {ctx.channel.name} in {ctx.guild.name} for {ctx.author} (message command fallback)")

    except Exception as e:
        print(f"❌ Error in !propose command: {e}")
        traceback.print_exc()
        try:
            if ctx.interaction and not ctx.interaction.response.is_done():
                await ctx.interaction.response.send_message(f"❌ An error occurred: {e}", ephemeral=True)
            else:
                await ctx.send(f"❌ An error occurred: {e}")
        except Exception: # Suppress errors during error reporting
            pass
