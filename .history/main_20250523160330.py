...
async def on_ready():
    ...
    for guild in bot.guilds:
        ...
        for channel_key in ["proposals", "voting", "results"]:
            ...
            if channel_name:
                # Calls proposals.get_or_create_channel() from the imported proposals module
                await proposals.get_or_create_channel(guild, channel_name)
...
async def vote(ctx, proposal_id: int, *args):
    ...
    if not isinstance(ctx.channel, discord.DMChannel):
        ...
        try:
            ...
            if proposal:
                options = proposals.extract_options_from_description(proposal['description'])
                if not options:
                    options = ["Yes", "No"]
...
async def setup_audit_log_channel(guild):
    ...
    overwrites = {
        ...
        guild.me: discord.PermissionOverwrite(
            read_messages=True, send_messages=True)  # Bot can send messages
    }
...
async def terminate_proposal(ctx, proposal_id: int):
    ...
    try:
        ...
        # Close the proposal
        from voting import close_proposal

        # Debug print to track execution
        print(f"DEBUG: Terminating proposal #{proposal_id} early")

        # Import directly from voting.py to avoid circular imports
