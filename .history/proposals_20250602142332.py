    ...
    try:
        ...
        if campaign_id and campaign: # Check if campaign object was fetched
            ...
            if campaign_status in ['setup', 'active']:
                final_initial_status = "Pending Approval" # Changed from "ApprovedScenario"
        ...
        if final_initial_status is None: # Safety net default
            ...
            final_initial_status = "Pending Approval" if campaign_id else "Pending Approval" # Changed from "ApprovedScenario"
    ...
    async def update_button_states(self, interaction: Optional[discord.Interaction] = None):
        ...
        if campaign['status'] == 'setup':
            ...
            if campaign['current_defined_scenarios'] > 0:
                self.start_next_button.disabled = False # Enable if at least one scenario defined
            else:
                self.start_next_button.label = "Define S1 First" # More specific label
                self.start_next_button.disabled = True

        elif campaign['status'] == 'active':
            # Fetch all proposals for the campaign to determine current state
            # Guild ID for fetching proposals should come from the campaign record.
            guild_id_for_proposals = campaign.get('guild_id')
            if guild_id_for_proposals:
                all_guild_proposals = await db.get_server_proposals(guild_id_for_proposals)
                proposals_in_campaign = [p for p in all_guild_proposals if p.get('campaign_id') == self.campaign_id]
                ...
                if active_voting_scenarios:
                    ...
                    last_closed_order = 0
                    if closed_scenarios:
                        last_closed_order = closed_scenarios[0].get('scenario_order', 0)
                    ...
                    if next_scenario_order_needed > campaign['num_expected_scenarios']:
                        ...
                        self.start_next_button.disabled = True
                        # Potentially the campaign should transition to 'completed' here or by a background task.
                        # For the button state, this is sufficient.
                    elif campaign['current_defined_scenarios'] >= next_scenario_order_needed:
                        # Next scenario is defined, so we can offer to start it.
                        self.start_next_button.label = f"Start Scenario {next_scenario_order_needed}"
                        self.start_next_button.disabled = False
                    else:
                        # Next scenario is needed but not yet defined.
                        self.start_next_button.label = f"Define S{next_scenario_order_needed} First"
                        self.start_next_button.disabled = True
            else: # Should not happen if campaign has a guild_id
                self.start_next_button.label = "Error: Guild ID Missing"
                self.start_next_button.disabled = True


        elif campaign['status'] == 'completed':
            self.start_next_button.label = "Campaign Completed"
            self.start_next_button.disabled = True
        elif campaign['status'] == 'rejected':
            self.start_next_button.label = "Campaign Rejected"
            self.start_next_button.disabled = True
        elif campaign['status'] == 'archived': # Assuming archived is a terminal state for buttons
            self.start_next_button.label = "Campaign Archived"
            self.start_next_button.disabled = True
        else: # pending_approval or other unexpected states
            self.start_next_button.label = f"Status: {campaign['status'].title()}"
            self.start_next_button.disabled = True

        # The interaction object is not used to edit the message here,
        # as _update_control_message (the typical caller) handles the message edit.
        # If this method were called directly with an interaction and an edit was desired,
        # it would need logic similar to the commented-out section from the original.
...
async def handle_start_campaign_or_next_scenario(interaction: discord.Interaction, campaign_id: int, bot: commands.Bot) -> tuple[bool, str]:
    # Correctly get the guild object. interaction.guild might be None if the interaction is from a DM.
    # However, campaign operations should be tied to a guild where the campaign exists.
    campaign_for_guild_id = await db.get_campaign(campaign_id)
    if not campaign_for_guild_id:
        return False, f"Campaign C#{campaign_id} not found."

    guild = bot.get_guild(campaign_for_guild_id['guild_id'])
    if not guild:
        print(f"ERROR: handle_start_campaign_or_next_scenario C#{campaign_id} called but guild G#{campaign_for_guild_id['guild_id']} not found by bot.")
        return False, "Internal error: Guild context not found for campaign."

    campaign = campaign_for_guild_id # Use the already fetched campaign data
    ...
    original_control_message: Optional[discord.Message] = None
    ...
    async def _update_control_message(status_message: str, color: discord.Color = discord.Color.blue(), success: bool = True):
        ...
        if not current_campaign_data_for_view:
            ...
            try:
                ...
                await original_control_message.edit(embed=error_embed, view=None) # Clear view on error
        ...
        # Manually call update_button_states on the new_view instance *before* sending it.
        # This replaces the direct manipulation of button properties within _update_control_message.
        # update_button_states itself will fetch the latest campaign data.
        await new_view.update_button_states() # This will use the latest data internally.
        ...
        final_color = color
        if current_campaign_data_for_view['status'] == 'active' and success: final_color = discord.Color.green()
        elif current_campaign_data_for_view['status'] == 'completed': final_color = discord.Color.gold()
        elif current_campaign_data_for_view['status'] == 'setup': final_color = discord.Color.light_grey()
        elif not success : final_color = discord.Color.orange()


        new_embed = discord.Embed(title=embed_title, description=embed_description, color=final_color)
    ...
    if campaign['status'] == 'setup':
        if campaign['current_defined_scenarios'] == 0:
            await _update_control_message("Cannot start: No scenarios defined yet.", color=discord.Color.orange(), success=False)
            return False, "Cannot start campaign: No scenarios have been defined yet."

        # Fetch all proposals once for this guild
        all_guild_proposals = await db.get_server_proposals(guild.id) # Make sure guild.id is valid

        first_scenario_data = next((p for p in all_guild_proposals if p.get('campaign_id') == campaign_id and p.get('scenario_order') == 1), None)

        if not first_scenario_data:
            err_msg = f"Error: First scenario (order 1) not found for C#{campaign_id}. Please define it."
            await _update_control_message(err_msg, color=discord.Color.red(), success=False)
            return False, err_msg

        if first_scenario_data['status'] not in ['ApprovedScenario', 'Voting']:
            msg = f"First scenario P#{first_scenario_data['proposal_id']} ('{first_scenario_data['title']}') is not ready (status: {first_scenario_data['status']}). It might need admin action."
            await _update_control_message(msg, color=discord.Color.orange(), success=False)
            return False, msg

        await db.update_campaign_status(campaign_id, 'active')
        await db.update_proposal_status(first_scenario_data['proposal_id'], 'Voting')

        # Refresh campaign data after status update for the embed
        campaign = await db.get_campaign(campaign_id)
        ...
        try:
            ...
            if proposals_channel:
                options = await db.get_proposal_options(first_scenario_data['proposal_id'])
                proposer_member = None
                try:
                    proposer_member = await guild.fetch_member(first_scenario_data['proposer_id'])
                except discord.NotFound:
                     print(f"WARN: Proposer U#{first_scenario_data['proposer_id']} for S1 P#{first_scenario_data['proposal_id']} C#{campaign_id} not found.")

                scenario_embed = utils.create_proposal_embed(
                    proposal_data=first_scenario_data, # Pass the full dictionary
                    proposer=proposer_member, # Pass the Member object or None
                    status='Voting', # Explicitly set status for embed
                    options=options, # Pass list of option strings
                    bot_instance=bot,
                    campaign_data=campaign
                )
                await proposals_channel.send(
                    content=f"📢 Campaign '{campaign['title']}' (C#{campaign_id}) is now ACTIVE!\\nScenario 1: '{first_scenario_data['title']}' (P#{first_scenario_data['proposal_id']}) is open for voting!",
                    embed=scenario_embed
                )
            else:
                print(f"WARN: Proposals channel '{proposals_channel_name}' not found for C#{campaign_id} S1 announcement.")

            voting_room_channel_name = utils.CHANNELS.get("voting", "voting-room") # Corrected key
            voting_room_channel = discord.utils.get(guild.text_channels, name=voting_room_channel_name)
            if voting_room_channel:
                # update_vote_tracking might need bot, guild, and full proposal_data
                await voting_utils.update_vote_tracking(bot, guild, first_scenario_data)
            else:
                print(f"WARN: Voting room '{voting_room_channel_name}' not found for C#{campaign_id} S1 tracking.")

            eligible_voters_list = await voting_utils.get_eligible_voters(guild, first_scenario_data)
            scenario_options_for_dm = await db.get_proposal_options(first_scenario_data['proposal_id'])
            if eligible_voters_list:
                dm_success_count, dm_fail_count = 0,0
                for member_to_dm in eligible_voters_list:
                    if member_to_dm.bot: continue
                    dm_sent = await voting.send_voting_dm(bot, member_to_dm, first_scenario_data, scenario_options_for_dm)
                    if dm_sent:
                        await db.add_voting_invite(first_scenario_data['proposal_id'], member_to_dm.id)
                        dm_success_count +=1
                    else:
                        dm_fail_count +=1
                print(f"INFO: Sent {dm_success_count} DMs (failed {dm_fail_count}) for C#{campaign_id} S1 P#{first_scenario_data['proposal_id']}.")
        except Exception as e_announce:
            print(f"ERROR announcing start of C#{campaign_id} S1 (P#{first_scenario_data['proposal_id']}): {e_announce}")
            traceback.print_exc()
            # Campaign is started, announcements might have failed. Log and continue.

        status_msg = f"Campaign '{campaign['title']}' started! Voting for Scenario 1: '{first_scenario_data['title']}' is active."
        await _update_control_message(status_msg, discord.Color.green(), success=True)
        return True, status_msg

    elif campaign['status'] == 'active':
        all_guild_proposals = await db.get_server_proposals(guild.id)
        proposals_in_campaign = [p for p in all_guild_proposals if p.get('campaign_id') == campaign_id]

        if not proposals_in_campaign: # Should not happen if campaign is active and S1 was started
            await _update_control_message("No scenarios found for this active campaign. This is an inconsistent state.", color=discord.Color.red(), success=False)
            return False, "No scenarios found for this active campaign. Inconsistent state."

        active_voting_scenarios = [p for p in proposals_in_campaign if p['status'] == 'Voting']
        if active_voting_scenarios:
            active_titles = ", ".join([f"'{s['title']}' (S#{s.get('scenario_order', 'N/A')})" for s in active_voting_scenarios])
            msg = f"Cannot start a new scenario. Scenario(s) {active_titles} are still 'Voting'."
            await _update_control_message(msg, color=discord.Color.orange(), success=False)
            return False, msg
        ...
        # If no scenarios are closed, but campaign is active, it means the first scenario hasn't closed yet.
        # The active_voting_scenarios check above should catch this if S1 is still voting.
        # If S1 has some other status (e.g. error), this indicates an issue.
        if not closed_scenarios and last_closed_order == 0 and campaign['current_defined_scenarios'] > 0 :
             # This condition means S1 was defined, but not closed, and not voting. Could be an issue or just pending closure.
             # Let's assume for now the check_expired_proposals task will handle closing it.
             # If the user presses "start next" now, and S1 is ApprovedScenario, something is wrong.
             # If S1 exists and is not 'Voting' or 'Closed', we should indicate that.
            first_scenario_data = next((p for p in proposals_in_campaign if p.get('campaign_id') == campaign_id and p.get('scenario_order') == 1), None)
            if first_scenario_data and first_scenario_data['status'] not in ['Voting', 'Closed']:
                msg = f"Scenario 1 ('{first_scenario_data['title']}') is not yet closed (status: {first_scenario_data['status']}). Please wait for it to complete."
                await _update_control_message(msg, color=discord.Color.orange(), success=False)
                return False, msg
            # If S1 does not exist, or is closed/voting, the logic proceeds.

        next_scenario_order_to_start = last_closed_order + 1

        if next_scenario_order_to_start > campaign['num_expected_scenarios']:
            await db.update_campaign_status(campaign_id, 'completed')
            campaign = await db.get_campaign(campaign_id) # Refresh for embed
            msg = f"All {campaign['num_expected_scenarios']} scenarios for '{campaign['title']}' completed. Campaign finished!"
            await _update_control_message(msg, color=discord.Color.gold(), success=True)
            # Also update audit log etc. for campaign completion
            audit_channel = discord.utils.get(guild.text_channels, name=utils.CHANNELS.get("audit", "audit-log"))
            if audit_channel:
                await audit_channel.send(f"🏁 **Campaign Completed**: C#{campaign_id} ('{campaign['title']}') has completed all scenarios.")
            return True, msg

        next_scenario_to_start_data = next((p for p in proposals_in_campaign if p.get('scenario_order') == next_scenario_order_to_start), None)

        if not next_scenario_to_start_data:
            if campaign['current_defined_scenarios'] < next_scenario_order_to_start:
                 msg = f"Scenario {next_scenario_order_to_start} for '{campaign['title']}' is not defined yet. Use 'Define Next Scenario'."
                 await _update_control_message(msg, color=discord.Color.orange(), success=False)
                 return False, msg
            else: # Defined but not found in proposals list for some reason - data integrity issue
                 msg = f"Error: S{next_scenario_order_to_start} for C#{campaign_id} not found despite being defined. Check data."
                 await _update_control_message(msg, color=discord.Color.red(), success=False)
                 return False, msg

        if next_scenario_to_start_data['status'] not in ['ApprovedScenario', 'Voting']:
            msg = f"Next scenario S#{next_scenario_order_to_start} ('{next_scenario_to_start_data['title']}') for C#{campaign_id} is not ready (status: {next_scenario_to_start_data['status']})."
            await _update_control_message(msg, color=discord.Color.orange(), success=False)
            return False, msg

        await db.update_proposal_status(next_scenario_to_start_data['proposal_id'], 'Voting')
        ...
        try:
            ...
            proposals_channel = discord.utils.get(guild.text_channels, name=proposals_channel_name)
            if proposals_channel:
                options = await db.get_proposal_options(next_scenario_to_start_data['proposal_id'])
                proposer_member = None
                try:
                     proposer_member = await guild.fetch_member(next_scenario_to_start_data['proposer_id'])
                except discord.NotFound:
                    print(f"WARN: Proposer U#{next_scenario_to_start_data['proposer_id']} for S{next_scenario_order_to_start} P#{next_scenario_to_start_data['proposal_id']} C#{campaign_id} not found.")

                scenario_embed = utils.create_proposal_embed(
                    proposal_data=next_scenario_to_start_data,
                ...
                await proposals_channel.send(
                    content=f"▶️ Campaign '{campaign['title']}' (C#{campaign_id}) - Scenario {next_scenario_order_to_start}: '{next_scenario_to_start_data['title']}' (P#{next_scenario_to_start_data['proposal_id']}) is now open for voting!",
                    embed=scenario_embed
                )
            else:
                print(f"WARN: Proposals channel '{proposals_channel_name}' not found for C#{campaign_id} S{next_scenario_order_to_start} ann.")

            voting_room_channel_name = utils.CHANNELS.get("voting", "voting-room")
            voting_room_channel = discord.utils.get(guild.text_channels, name=voting_room_channel_name)
            if voting_room_channel:
                await voting_utils.update_vote_tracking(bot, guild, next_scenario_to_start_data)
            else:
                print(f"WARN: Voting room '{voting_room_channel_name}' not found for C#{campaign_id} S{next_scenario_order_to_start} track.")

            eligible_voters_list = await voting_utils.get_eligible_voters(guild, next_scenario_to_start_data)
            scenario_options_for_dm = await db.get_proposal_options(next_scenario_to_start_data['proposal_id'])
            if eligible_voters_list:
                dm_success_count, dm_fail_count = 0,0
                for member_to_dm in eligible_voters_list:
                    if member_to_dm.bot: continue
                    dm_sent = await voting.send_voting_dm(bot, member_to_dm, next_scenario_to_start_data, scenario_options_for_dm)
                    if dm_sent:
                        await db.add_voting_invite(next_scenario_to_start_data['proposal_id'], member_to_dm.id)
                        dm_success_count += 1
                    else:
                        dm_fail_count += 1
                print(f"INFO: Sent {dm_success_count} DMs (failed {dm_fail_count}) for C#{campaign_id} S{next_scenario_order_to_start} P#{next_scenario_to_start_data['proposal_id']}.")
        except Exception as e_announce_next:
            print(f"ERROR announcing start of C#{campaign_id} S{next_scenario_order_to_start} (P#{next_scenario_to_start_data['proposal_id']}): {e_announce_next}")
            traceback.print_exc()

        status_msg = f"Voting for Scenario {next_scenario_order_to_start}: '{next_scenario_to_start_data['title']}' in campaign '{campaign['title']}' is now active."
        if next_scenario_order_to_start == campaign['num_expected_scenarios']:
            status_msg += " This is the final expected scenario."

        await _update_control_message(status_msg, discord.Color.green(), success=True)
        return True, status_msg

    elif campaign['status'] in ['completed', 'rejected', 'archived']:
        msg = f"Campaign '{campaign['title']}' is already '{campaign['status']}'. No further start/progress actions available."
        await _update_control_message(msg, color=discord.Color.greyple(), success=False)
        return False, msg

    else: # Should not be reached if all statuses handled (e.g. pending_approval)
        msg = f"Campaign '{campaign['title']}' is in an unhandled state: '{campaign['status']}'. No action taken."
        await _update_control_message(msg, color=discord.Color.red(), success=False)
        return False, msg
