    ...
    async def submit_vote_callback(self, interaction: discord.Interaction):
        ...
        if self.is_abstain_vote: # Handled by AbstainButton's callback directly calling finalize_vote
            ...
            if self.user_remaining_tokens == 0: # No tokens left, auto-invest 0
                ...
                # Plurality option_callback should have deferred, so we need to handle that case.
                try:
                    ...
                    # If the interaction has already been responded to, we can't send a modal
                    # Instead, we'll send a followup message asking for token investment
                    await interaction.followup.send(f"Please enter the number of tokens you want to invest (0-{self.user_remaining_tokens}):", ephemeral=True)
                    # Ideally here we would create a new way to collect token input, but as a temporary fix
                    # we'll use 1 token as default to prevent blocking the user
...
class PluralityVoteView(BaseVoteView):
    ...
    async def option_callback(self, interaction: discord.Interaction):
        ...
        # Defer the interaction with thinking=True as we'll do some processing
        # and the final response will be an edit in finalize_vote.
            if not interaction.response.is_done():
                await interaction.response.defer(ephemeral=True, thinking=True)
            else:
            # If already responded (e.g. by a quick double click that got through initial checks)
            # we can't defer again. Proceed, but log this.
