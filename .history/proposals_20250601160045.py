...
async def handle_start_campaign_or_next_scenario(interaction: discord.Interaction, campaign_id: int, bot: commands.Bot) -> tuple[bool, str]:
    ...
    if campaign['status'] == 'setup':
        ...
        proposals = await db.get_server_proposals(campaign['guild_id'])
        ...
        if not first_scenario:
            await _update_control_message(f"Error: First scenario (order 1) not found for C#{campaign_id}. Please define it.", discord.Color.red())
            return False, "First scenario (order 1) not found. Please define it."

        if first_scenario['status'] not in ['ApprovedScenario', 'Voting']:
            msg = f"First scenario P#{first_scenario['proposal_id']} is not ready (status: {first_scenario['status']}). It needs to be approved or is in an unexpected state."
        ...
        current_campaign_data_after_update = await db.get_campaign(campaign_id) # For accurate counts
