...
async def propose(ctx: commands.Context):
    """Command to propose a new idea"""
    try:
        print(f"🔍 {ctx.author} invoked !propose in {ctx.guild.name}")
        # Pass the Context object to the view
        view = ProposalView(ctx)
        # Store the message object the view is attached to for potential later editing on timeout
        # view.message = await ctx.send("Click the button below to create a proposal:", view=view)
        await ctx.send("Click the button below to create a proposal:", view=view)
        print(f"✅ ProposalView sent to {ctx.channel.name} in {ctx.guild.name}")
    except Exception as e:
        print(f"❌ Error in !propose command: {e}")
        await ctx.send(f"❌ An error occurred while trying to create a proposal: {e}")
