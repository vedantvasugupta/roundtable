...
async def initiate_campaign_stage_voting(guild: discord.Guild, campaign_id: int, scenario_proposal_ids: List[int], bot_instance: commands.Bot) -> Tuple[bool, str]:
    """
    Initiates voting for a list of scenarios in a campaign stage.
    Sets them to 'Voting', announces them, and triggers batched DM sending.
    """
    successful_inits = 0
    failed_inits = 0
    overall_message = []
    ...
    if not scenario_proposal_ids:
        return False, "No scenario proposal IDs provided for campaign stage."

    # 1. Set all relevant scenarios to 'Voting' and announce them
    for proposal_id in scenario_proposal_ids:
        try:
            proposal_details = await db.get_proposal(proposal_id)
            if not proposal_details:
                overall_message.append(f"Scenario #{proposal_id} not found.")
                failed_inits += 1
                continue

            if proposal_details.get('campaign_id') != campaign_id:
                overall_message.append(f"Scenario #{proposal_id} does not belong to Campaign C#{campaign_id}.")
                failed_inits +=1
                continue

            if proposal_details['status'] == "Voting":
                overall_message.append(f"Scenario #{proposal_id} is already 'Voting'.")
                successful_inits += 1 # Count as success if already voting
                continue

            # Only proceed if status allows (e.g. 'ApprovedScenario')
            if proposal_details['status'] != "ApprovedScenario":
                overall_message.append(f"Scenario #{proposal_id} is not in 'ApprovedScenario' status (is {proposal_details['status']}). Cannot start voting.")
                failed_inits +=1
                continue

            await db.update_proposal_status(proposal_id, "Voting")
            current_status_for_embed = "Voting"

            # Announce in 'proposals' channel (similar to initiate_voting_for_proposal)
            proposals_channel_name = utils.CHANNELS.get("proposals", "proposals")
            proposals_channel = await utils.get_or_create_channel(guild, proposals_channel_name, bot_instance.user.id)
            if proposals_channel:
                options = await db.get_proposal_options(proposal_id)
                ...
                embed = utils.create_proposal_embed(
                    ...
                    proposal_details['voting_mechanism'], proposal_details['deadline'], current_status_for_embed, option_names,
                    hyperparameters=proposal_details.get('hyperparameters'),
                    campaign_id=campaign_id, scenario_order=proposal_details.get('scenario_order')
                )
                await proposals_channel.send(content=f"🎉 Voting has started for Campaign C#{campaign_id} Scenario S#{proposal_details.get('scenario_order')} (Proposal #{proposal_id})!", embed=embed)

            # Update voting room tracking (similar to initiate_voting_for_proposal)
            voting_room_channel_name = utils.CHANNELS.get("voting-room", "voting-room")
            voting_room_channel = await utils.get_or_create_channel(guild, voting_room_channel_name, bot_instance.user.id)
            ...
            successful_inits += 1
            overall_message.append(f"Scenario #{proposal_id} (S#{proposal_details.get('scenario_order')}) voting started.")

        except Exception as e:
            failed_inits += 1
            error_msg = f"Error initiating Scenario #{proposal_id} for C#{campaign_id}: {e}"
            print(error_msg)
            traceback.print_exc()
            overall_message.append(error_msg)

    if successful_inits == 0 and failed_inits > 0:
        return False, f"Failed to initiate any scenarios for Campaign C#{campaign_id}. Details: {'; '.join(overall_message)}"

    # 2. Trigger batched DM sending for successfully initiated scenarios
    # Only send DMs for scenarios that are now confirmed to be in 'Voting' status.
    # Re-fetch to be sure, or trust the successful_inits list if precise enough.
    # For simplicity, let's assume all in scenario_proposal_ids that didn't error out are candidates.
    # A more robust way would be to collect proposal_ids that successfully transitioned to 'Voting'.

    successfully_started_scenario_ids = []
    for proposal_id in scenario_proposal_ids: # Iterate again to check final status
        # This is a bit redundant, could be optimized by collecting IDs above
        # but ensures we only try to DM for proposals truly in 'Voting'
        prop_check = await db.get_proposal(proposal_id)
        if prop_check and prop_check.get('campaign_id') == campaign_id and prop_check.get('status') == 'Voting':
            successfully_started_scenario_ids.append(proposal_id)

    if successfully_started_scenario_ids:
        # This function will handle fetching voters and then calling out to voting.py
        dm_batch_success, dm_batch_message = await send_batched_campaign_dms(guild, campaign_id, successfully_started_scenario_ids, bot_instance)
        overall_message.append(f"DM Batching: {dm_batch_message}")
    else:
        overall_message.append("No scenarios were successfully set to 'Voting' to send DMs for.")

    final_report = f"Campaign C#{campaign_id} stage processing: {successful_inits} scenarios initiated, {failed_inits} failed. {'; '.join(overall_message)}"
    return successful_inits > 0, final_report
...
async def send_batched_campaign_dms(guild: discord.Guild, campaign_id: int, scenario_proposal_ids: List[int], bot_instance: commands.Bot) -> Tuple[bool, str]:
    """
    Fetches all data needed and calls a function in voting.py to send batched DMs to each eligible campaign voter.
    """
    try:
        if not scenario_proposal_ids:
