        ...
        if proposal_id:
            ...
            if requires_approval:
                ...
                if admin_channel:
                    ...
                    # Ensure deadline_db_str is a valid datetime string or datetime object for format_deadline
                    # If deadline_db_str from modal is like "7d", it needs conversion before format_deadline if it expects datetime obj
                    # However, create_proposal_embed (called by _perform_approve) handles string conversion internally.
                    # For consistency, format_deadline should also be robust or we ensure correct type.
                    # Let's assume deadline_db_str is an ISO format string from the modal submission process (actual_deadline_datetime.strftime('%Y-%m-%d %H:%M:%S.%f'))
...
async def _perform_approve_proposal_action(interaction_or_ctx: Union[discord.Interaction, commands.Context], proposal_id: int, bot_instance: commands.Bot) -> tuple[bool, str]:
    ...
    try:
        ...
        if audit_channel:
            await audit_channel.send(f"✅ **Proposal Approved**: #{proposal_id} ('{proposal['title']}') approved by {user.mention}. Voting started.")

        return True, f"✅ Proposal #{proposal_id} ('{proposal['title']}') has been approved and voting has started!"
