...
async def process_vote(user_id, proposal_id, vote_data):
    ...
    try:
        ...
        if show_vote_count and server_info:
            await update_vote_count_message(server_info, proposal_id)
        ...
        if eligible_voters_role.lower() == "everyone" and server_info and 'member_count' in server_info:
            eligible_voters = await get_eligible_voters(server_info, proposal)
            if len(all_votes) >= len(eligible_voters):
                # All eligible voters have voted, close the proposal
                results = await close_proposal(proposal_id)
                await close_and_announce_results(server_info, proposal, results)
