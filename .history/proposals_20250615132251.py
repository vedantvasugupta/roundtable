...
class BaseProposalModal(discord.ui.Modal):
    ...
    async def common_on_submit(self, interaction: discord.Interaction, specific_hyperparameters: Dict[str, Any]):
        ...
        print("DEBUG: Entered common_on_submit")
        try:
            title = self.proposal_title_input.value
            print(f"DEBUG: Proposal Title: {title}")
            ...
            print(f"DEBUG: Proposal Description: {description}")

            options_text = self.options_input.value
            deadline_str = self.deadline_input.value or "7d"
            print(f"DEBUG: Raw Options Text: '{options_text}'")
            print(f"DEBUG: Raw Deadline String: '{deadline_str}'")
            ...
            if deadline_seconds is None:
                ...
                return
            print(f"DEBUG: Parsed Deadline (seconds): {deadline_seconds}")
            ...
            print(f"DEBUG: Deadline for DB: {deadline_db_str}")

            # Interaction is already deferred by the calling on_submit method.

            # Call _create_new_proposal_entry
            print("DEBUG: Calling _create_new_proposal_entry...")
            ...
            print(f"DEBUG: _create_new_proposal_entry returned proposal_id: {proposal_id}")
            ...
            if interaction.response.is_done():
                ...
                # This 'else' block is now less likely to be hit, but kept as a fallback.
                try:
...
class PluralityProposalModal(BaseProposalModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        print("DEBUG: PluralityProposalModal on_submit triggered.")
        await interaction.response.defer(ephemeral=True, thinking=True)  # Defer first
        try:
            hyperparameters = {}
            ...
            if allow_abstain_str in ["yes", "y", ""]:
                ...
                await interaction.followup.send("Invalid input for 'Allow Abstain'. Please use 'yes' or 'no', or leave blank for 'yes'.", ephemeral=True)
                return
            ...
            if threshold_str:
                try:
                    ...
                    if not (0 <= threshold <= 100):
                        await interaction.followup.send("Winning threshold must be a percentage between 0 and 100.", ephemeral=True)
                        return
                    hyperparameters["winning_threshold_percentage"] = threshold
                except ValueError:
                    await interaction.followup.send("Invalid input for winning threshold. Please enter a whole number (e.g., 40).", ephemeral=True)
                    return
...
class BordaProposalModal(BaseProposalModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        print("DEBUG: BordaProposalModal on_submit triggered.")
        await interaction.response.defer(ephemeral=True, thinking=True)  # Defer first
        try:
            hyperparameters = {}
            allow_abstain_str = self.allow_abstain_input.value.strip().lower()
...
class ApprovalProposalModal(BaseProposalModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        print("DEBUG: ApprovalProposalModal on_submit triggered.")
...
class RunoffProposalModal(BaseProposalModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        print("DEBUG: RunoffProposalModal on_submit triggered.")
        await interaction.response.defer(ephemeral=True, thinking=True)
        try:
            hyperparameters = {}
            allow_abstain_str = self.allow_abstain_input.value.strip().lower()
...
class DHondtProposalModal(BaseProposalModal):
    def __init__(self, interaction: discord.Interaction, mechanism_name: str, campaign_id: Optional[int] = None, scenario_order: Optional[int] = None, title_prefix: str = "New"):
        ...
        self.seats_to_allocate_input = discord.ui.TextInput(
            label="Number of Seats to Allocate",
            placeholder="Enter the number of seats (e.g., 5)",
            required=True,
            max_length=3
        )
        self.add_item(self.seats_to_allocate_input)

    async def on_submit(self, interaction: discord.Interaction):
        print("DEBUG: DHondtProposalModal on_submit triggered.")
        await interaction.response.defer(ephemeral=True, thinking=True)
        try:
            hyperparameters = {}
            allow_abstain_str = self.allow_abstain_input.value.strip().lower()
            ...
            seats_str = self.seats_to_allocate_input.value.strip()
            if seats_str:
                try:
                    seats = int(seats_str)
                    if not (seats > 0):
                        await interaction.followup.send("Number of seats must be a positive number.", ephemeral=True)
                        return
                    hyperparameters["seats_to_allocate"] = seats
                except ValueError:
                    await interaction.followup.send("Invalid input for seats. Please enter a whole number.", ephemeral=True)
                    return
            else:
                await interaction.followup.send("Number of seats to allocate is required for D'Hondt.", ephemeral=True)
                return
...
async def _create_new_proposal_entry(interaction: discord.Interaction, title: str, description: str, mechanism_name: str, options: List[str], deadline_db_str: str, hyperparameters: Optional[Dict[str, Any]] = None, campaign_id: Optional[int] = None, scenario_order: Optional[int] = None) -> Optional[int]:
    ...
    """
    print("DEBUG: Inside _create_new_proposal_entry function.")
    try:
        guild_id = interaction.guild_id
        if not guild_id:
            print("ERROR: Guild ID not found in interaction.")
        ...
        print("DEBUG: Fetching constitutional variables...")
        const_vars = await db.get_constitutional_variables(guild_id)
        print(f"DEBUG: Constitutional variables fetched: {const_vars}")
        ...
        print(f"DEBUG: Eligible Proposers Role: {eligible_proposers_role_name}")
        ...
        if eligible_proposers_role_name != "everyone":
            print(f"DEBUG: Checking if user has role '{eligible_proposers_role_name}'...")
            ...
            if not eligible_role or eligible_role not in interaction.user.roles:
                print(f"DEBUG: User {interaction.user.name} does not have the required role.")
                ...
                return None
        print("DEBUG: User is eligible to propose.")

        # Determine initial status
        initial_status = "Pending Approval" if requires_approval else "Voting"
        print(f"DEBUG: Initial proposal status will be: {initial_status}")

        # Convert hyperparameters to JSON string for DB
        hyperparameters_json = json.dumps(hyperparameters) if hyperparameters else "{}"
        print(f"DEBUG: Hyperparameters JSON for DB: {hyperparameters_json}")

        # Insert proposal into the database
        print("DEBUG: Calling db.create_proposal...")
        proposal_id = await db.create_proposal(
            server_id=guild_id,
            proposer_id=interaction.user.id,
            title=title,
            description=description,
            voting_mechanism=mechanism_name, # Corrected parameter name
            status=initial_status,
            deadline=deadline_db_str,
            # options=options, # Options are added separately
            hyperparameters=hyperparameters_json,
            campaign_id=campaign_id,
            scenario_order=scenario_order
        )
        print(f"DEBUG: db.create_proposal returned proposal_id: {proposal_id}")

        if not proposal_id:
            print("ERROR: db.create_proposal failed to return a proposal ID.")
            await interaction.followup.send("There was a critical error trying to save the proposal to the database.", ephemeral=True)
            return None

        # Add options to the separate proposal_options table
        print(f"DEBUG: Adding options for proposal #{proposal_id}...")
        await db.add_proposal_options(proposal_id, options)
        print("DEBUG: Options added successfully.")

        print("DEBUG: Proposal created successfully in DB.")
        # --- Handle post-creation actions based on status ---

        if initial_status == "Pending Approval":
            # Notify user that it's pending approval
            print("DEBUG: Proposal is pending approval. Sending user feedback.")
            await interaction.followup.send(
            ...
            print("DEBUG: Sending notification to admin channel for approval.")
            ...
            print("DEBUG: Proposal is immediately active for voting. Sending user feedback.")
            await interaction.followup.send(
                f"✅ Proposal '{title}' (ID: #{proposal_id}) has been created and voting is now open!",
                ephemeral=True
            )
            print("DEBUG: Fetching full proposal data for DM distribution.")
            # We need the full proposal dict to send DMs
            proposal = await db.get_proposal(proposal_id)
            if proposal:
                print("DEBUG: Distributing voting DMs...")
                await voting_utils.distribute_voting_dms(interaction.client, proposal)
                print("DEBUG: Finished distributing DMs.")
            else:
