...
async def get_proposal_scenario_order(proposal_id: int) -> Optional[int]:
    """Retrieve the scenario_order for a given proposal_id."""
    async with aiosqlite.connect(DB_NAME) as db:
        try:
            cursor = await db.execute("SELECT scenario_order FROM proposals WHERE proposal_id = ?", (proposal_id,))
            row = await cursor.fetchone()
            await cursor.close()
            return row[0] if row else None
        except Exception as e:
            print(f"Error getting scenario_order for proposal P#{proposal_id}: {e}")
            return None