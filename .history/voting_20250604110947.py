    ...
    async def submit_vote_callback(self, interaction: discord.Interaction):
        ...
        if self.is_abstain_vote: # Handled by AbstainButton's callback directly calling finalize_vote
            ...
            if self.user_remaining_tokens == 0: # No tokens left, auto-invest 0
                ...
                # However, Plurality option_callback might have already deferred the interaction.
                try:
                    ...
                    # Instead, we'll use a default token value and proceed
                    print(f"WARNING: Interaction already responded to for P#{self.proposal_id} U#{self.user_id}. Using default token investment of 1.")
                    await interaction.followup.send(f"Using default token investment of 1 (out of {self.user_remaining_tokens} available).", ephemeral=True)
