...
def create_proposal_embed(proposal_id, proposer, title, description, voting_mechanism, deadline, status, options, results=None, requires_approval=None, hyperparameters: Optional[Dict[str, Any]] = None, campaign_id: Optional[int] = None, scenario_order: Optional[int] = None, campaign_title: Optional[str] = None):
    ...
    if isinstance(hyperparameters, dict): # Ensure hyperparameters is a dictionary
        hyperparams_text_parts = []
        # Plurality Specific
        if voting_mechanism.lower() == "plurality":
            if hyperparameters.get("allow_abstain") is not None: # Explicitly check if key exists
                hyperparams_text_parts.append(f"- Allow Abstain: {'Yes' if hyperparameters.get('allow_abstain', False) else 'No'}")
        ...
        # D'Hondt Specific
            ...
            if hyperparameters.get("allow_abstain") is not None:
                hyperparams_text_parts.append(f"- Allow Abstain: {'Yes' if hyperparameters.get('allow_abstain', False) else 'No'}")
