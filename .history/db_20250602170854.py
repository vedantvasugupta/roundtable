...
async def get_proposals_by_campaign_id(campaign_id: int, guild_id: Optional[int] = None) -> List[Dict[str, Any]]:
    """Retrieve all proposals (scenarios) for a given campaign_id, optionally filtered by guild_id."""
    async with get_db() as conn:
        if guild_id:
            cursor = await conn.execute(
                "SELECT * FROM proposals WHERE campaign_id = ? AND server_id = ?", (campaign_id, guild_id)
            )
        else:
            cursor = await conn.execute(
                "SELECT * FROM proposals WHERE campaign_id = ?", (campaign_id,)
            )
        rows = await cursor.fetchall()
        return [dict(row) for row in rows]
