...
async def handle_start_campaign_or_next_scenario(interaction: discord.Interaction, campaign_id: int, bot: commands.Bot) -> tuple[bool, str]:
    ...
    async def _update_control_message(status_message: str, color: discord.Color = discord.Color.blue()):
        if original_control_message:
            ...
            await new_view.update_button_states() # Crucial: refresh button states based on new DB state

            current_campaign_data = await db.get_campaign(campaign_id) # Re-fetch for latest counts
            creator = await guild.fetch_member(current_campaign_data['creator_id'])

            embed_title = f"Campaign Management: '{current_campaign_data['title']}' (ID: C#{campaign_id})"
            embed_description = f"**Creator:** {creator.mention if creator else f'ID: {current_campaign_data['creator_id']}'}\n"
            embed_description += f"**Description:** {current_campaign_data['description'] or 'Not provided.'}\n"
            embed_description += f"**Total Scenarios Expected:** {current_campaign_data['num_expected_scenarios']}\n"
            embed_description += f"**Currently Defined:** {current_campaign_data['current_defined_scenarios']}"

            new_embed = discord.Embed(title=embed_title, description=embed_description, color=color)
            new_embed.add_field(name="Status", value=current_campaign_data['status'].title(), inline=True)
            new_embed.add_field(name="Last Action", value=status_message, inline=False)
            ...
            try:
                ...
                print(f"ERROR: Failed to edit control message for C#{campaign_id}: {e_edit_msg}")
    ...
    if campaign['status'] == 'setup':
        ...
        # Find the first scenario (scenario_order = 1)
        proposals = await db.get_server_proposals(campaign['guild_id']) # Fetch all for the guild
        first_scenario = next((p for p in proposals if p.get('campaign_id') == campaign_id and p.get('scenario_order') == 1), None)

        if not first_scenario:
            await _update_control_message(f"Error: First scenario (order 1) not found for C#{campaign_id}.", discord.Color.red())
            return False, "First scenario (order 1) not found. Please define it."

        # Check if first scenario is approved (it should be if requires_approval was handled correctly during its creation)
        if first_scenario['status'] not in ['ApprovedScenario', 'Voting']: # 'Voting' if somehow already set
             # This implies an issue, perhaps it was rejected or still pending manual approval for some reason.
            msg = f"First scenario P#{first_scenario['proposal_id']} is not ready (status: {first_scenario['status']}). It needs to be approved."
            await _update_control_message(msg, discord.Color.orange())
            return False, msg

        # Update campaign status to active
        await db.update_campaign_status(campaign_id, 'active')
        # Update scenario status to Voting
        await db.update_proposal_status(first_scenario['proposal_id'], 'Voting')

        # Trigger announcements (similar to _perform_approve_proposal_action for standalone)
        try:
            # This part needs to be robust, possibly extracted to a shared utility function
            ...
            if proposals_channel:
                ...
                proposer_member = guild.get_member(first_scenario['proposer_id'])
                scenario_embed = utils.create_proposal_embed(
                    first_scenario['proposal_id'], proposer_member, first_scenario['title'], first_scenario['description'],
                    first_scenario['voting_mechanism'], first_scenario['deadline'], 'Voting', options,
                    hyperparameters=first_scenario.get('hyperparameters'), campaign_id=campaign_id, scenario_order=1
                )
                await proposals_channel.send(content=f"📢 Campaign '{campaign['title']}' - Scenario 1 ('{first_scenario['title']}') is now open for voting!", embed=scenario_embed)
            ...
            if voting_room_channel:
                await voting_utils.update_vote_tracking(guild, first_scenario['proposal_id'])

            # Send DMs for the first scenario
            eligible_voters_list = await voting_utils.get_eligible_voters(guild, first_scenario)
            scenario_options = await db.get_proposal_options(first_scenario['proposal_id'])
            if eligible_voters_list:
                for member_to_dm in eligible_voters_list:
                    if member_to_dm.bot: continue
                    dm_sent = await voting.send_voting_dm(member_to_dm, first_scenario, scenario_options)
                    if dm_sent: await db.add_voting_invite(first_scenario['proposal_id'], member_to_dm.id)
        except Exception as e_announce:
            print(f"ERROR announcing start of C#{campaign_id} S1 (P#{first_scenario['proposal_id']}): {e_announce}")
            # Campaign is started, but announcements might have failed. Log and continue.

        status_msg = f"Campaign started. Voting for Scenario 1 ('{first_scenario['title']}') is active."
        await _update_control_message(status_msg, discord.Color.green())
        return True, status_msg

    # --- Start Next Scenario Logic (status 'active') ---
    elif campaign['status'] == 'active':
        # Determine current active scenario and if it's closed
        # This requires more complex tracking: which scenario is currently active for this campaign?
        # Option 1: Add `current_scenario_order` to `campaigns` table.
        # Option 2: Query proposals table for this campaign_id where status is 'Voting'.
        # Option 3: Query proposals for campaign, sort by order, find first non-closed, non-pending.

        # Simplified approach for now: Find the highest order scenario for this campaign that is 'Closed'.
        # Then try to start the next one (closed_order + 1).
        ...
        if not proposals_in_campaign:
            await _update_control_message("No scenarios found for this active campaign.", discord.Color.red())
        ...
        active_scenarios = [p for p in proposals_in_campaign if p['status'] == 'Voting']

        if active_scenarios:
            active_scenario_titles = ", ".join([f"'{s['title']}' (S#{s['scenario_order']})" for s in active_scenarios])
            msg = f"Cannot start next scenario. Scenario(s) {active_scenario_titles} are still active."
            await _update_control_message(msg, discord.Color.orange())
            return False, msg
        ...
        if next_scenario_order_to_start > campaign['num_expected_scenarios']:
            # All expected scenarios seem to be done. Mark campaign completed.
            await db.update_campaign_status(campaign_id, 'completed')
            msg = "All expected scenarios completed. Campaign finished."
        ...
        if not next_scenario_to_start:
            if campaign['current_defined_scenarios'] < next_scenario_order_to_start <= campaign['num_expected_scenarios']:
                 msg = f"Scenario {next_scenario_order_to_start} is not defined yet. Please define it first."
                 await _update_control_message(msg, discord.Color.orange())
                 return False, msg
            else: # Should be caught by num_expected_scenarios check or implies data issue
                 msg = f"Error: Next scenario (order {next_scenario_order_to_start}) not found for C#{campaign_id}."
                 await _update_control_message(msg, discord.Color.red())
                 return False, msg

        if next_scenario_to_start['status'] not in ['ApprovedScenario', 'Voting']:
            msg = f"Next scenario S#{next_scenario_order_to_start} ('{next_scenario_to_start['title']}') is not ready (status: {next_scenario_to_start['status']})."
            await _update_control_message(msg, discord.Color.orange())
            return False, msg

        # Start the next scenario
        await db.update_proposal_status(next_scenario_to_start['proposal_id'], 'Voting')
        # Trigger announcements (similar to above)
        try:
            proposals_channel_name = utils.CHANNELS.get("proposals", "proposals")
            proposals_channel = discord.utils.get(guild.text_channels, name=proposals_channel_name)
            if proposals_channel:
                options = await db.get_proposal_options(next_scenario_to_start['proposal_id'])
                proposer_member = guild.get_member(next_scenario_to_start['proposer_id'])
                scenario_embed = utils.create_proposal_embed(
                    next_scenario_to_start['proposal_id'], proposer_member, next_scenario_to_start['title'], next_scenario_to_start['description'],
                    next_scenario_to_start['voting_mechanism'], next_scenario_to_start['deadline'], 'Voting', options,
                    hyperparameters=next_scenario_to_start.get('hyperparameters'), campaign_id=campaign_id, scenario_order=next_scenario_order_to_start
                )
                await proposals_channel.send(content=f"📢 Campaign '{campaign['title']}' - Scenario {next_scenario_order_to_start} ('{next_scenario_to_start['title']}') is now open for voting!", embed=scenario_embed)

            voting_room_channel_name = utils.CHANNELS.get("voting", "voting-room")
            voting_room_channel = discord.utils.get(guild.text_channels, name=voting_room_channel_name)
            if voting_room_channel:
                await voting_utils.update_vote_tracking(guild, next_scenario_to_start['proposal_id'])

            eligible_voters_list = await voting_utils.get_eligible_voters(guild, next_scenario_to_start)
            scenario_options = await db.get_proposal_options(next_scenario_to_start['proposal_id'])
            if eligible_voters_list:
                for member_to_dm in eligible_voters_list:
                    if member_to_dm.bot: continue
                    dm_sent = await voting.send_voting_dm(member_to_dm, next_scenario_to_start, scenario_options)
                    if dm_sent: await db.add_voting_invite(next_scenario_to_start['proposal_id'], member_to_dm.id)
        ...
        status_msg = f"Voting for Scenario {next_scenario_order_to_start} ('{next_scenario_to_start['title']}') is now active."
        # Check if this was the last expected scenario
        if next_scenario_order_to_start == campaign['num_expected_scenarios']:
            status_msg += " This is the final scenario for the campaign."
            # Campaign status remains 'active' until this final scenario closes.
            # The check_proposal_deadlines_task will eventually close it, then this button could mark campaign 'completed'.

        await _update_control_message(status_msg, discord.Color.green())
        return True, status_msg

    elif campaign['status'] == 'completed':
        msg = "This campaign has already been completed."
        await _update_control_message(msg, discord.Color.gold())
        return False, msg
    else:
