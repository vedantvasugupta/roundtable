...
class PluralityVoteView(BaseVoteView):
    ...
    async def option_callback(self, interaction: discord.Interaction):
        ...
        # Determine selected option from the interaction
        # Assuming custom_id format like "plurality_option_PROPOSALID_INDEX" or button label is option text
        ...
        if clicked_component and hasattr(clicked_component, 'label'):
            ...
            # Fallback or error if label not found (should ideally not happen if buttons are set up correctly)
            print(f"ERROR: PluralityVoteView - Could not determine selected option from custom_id {custom_id} for P#{self.proposal_id}")
            if not interaction.response.is_done():
        ...
        should_send_modal = (
            ...
            # self.is_abstain_vote is False here as this is an option callback
        )
        ...
        if not should_send_modal:
            if not interaction.response.is_done():
                ...
                print(f"WARNING: PluralityVoteView.option_callback trying to defer an already responded interaction. P#{self.proposal_id} U#{self.user_id}")
        # If should_send_modal is True, we do *not* defer here because submit_vote_callback will call interaction.response.send_modal()

        # Update button styles: disable all option buttons and mark the selected one
        for item in self.children:
            if isinstance(item, discord.ui.Button) and item.custom_id != f"abstain_{self.proposal_id}": # Don't disable abstain button here
                if hasattr(item, 'label') and item.label == self.selected_option:
                    item.style = discord.ButtonStyle.success
                else:
                    item.style = discord.ButtonStyle.secondary
                item.disabled = True

        # If we deferred (i.e., not sending a modal), edit the message to show updated button styles.
        if not should_send_modal:
            if interaction.response.is_done(): # It should be done if we deferred successfully
                try:
                    await interaction.edit_original_response(view=self)
                except discord.HTTPException as e:
                    print(f"Error editing message in PluralityVoteView.option_callback after defer: {e}")

        # Proceed to the general submission logic in BaseVoteView
...
class RankedVoteView(BaseVoteView):
    ...
    async def rank_callback(self, interaction: discord.Interaction):
        ...
        # The interaction.data['values'] will contain the selected ranks (list of option strings)
        self.selected_rankings = interaction.data.get('values', [])
        self.selected_mechanism_vote_data = self.get_mechanism_vote_data() # Update based on selection

        # Disable the select menu after selection
        for item in self.children:
            if isinstance(item, discord.ui.Select) and item.custom_id == interaction.data.get('custom_id'):
                item.disabled = True
                item.placeholder = "Ranking submitted. Click other items or Abstain if needed."
                break
        ...
        should_send_modal = (
            ...
            self.user_remaining_tokens is not None and
            self.user_remaining_tokens > 0
            # self.is_abstain_vote is False here as this is a rank_callback for a select menu
        )
        ...
        if not should_send_modal:
            if not interaction.response.is_done():
                ...
                print(f"WARNING: RankedVoteView.rank_callback trying to defer an already responded interaction. P#{self.proposal_id} U#{self.user_id}")
        # If should_send_modal is True, we do *not* defer here.

        # If we deferred, edit the message to show the updated (disabled) select menu.
        if not should_send_modal:
            if interaction.response.is_done(): # Should be done if we deferred successfully
                try:
                    await interaction.edit_original_response(view=self)
                except discord.HTTPException as e:
...
class ApprovalVoteView(BaseVoteView):
    ...
    def __init__(self, proposal_id: int, options: List[str], user_id: int, allow_abstain: bool = True, campaign_id: Optional[int] = None, campaign_details: Optional[Dict[str, Any]] = None, user_remaining_tokens: Optional[int] = None):
        super().__init__(proposal_id, options, user_id, allow_abstain, campaign_id, campaign_details, user_remaining_tokens)
        self.current_approvals: set[str] = set() # Store currently approved options
        # The submit button is added in add_mechanism_items
    ...
    def add_mechanism_items(self):
        ...
        for i, option_text in enumerate(self.options):
            button = discord.ui.Button(
                label=option_text,
                style=discord.ButtonStyle.secondary, # Initial style
                custom_id=f"approve_option_{self.proposal_id}_{i}"
            )
            # Assign to a generic option_button_callback that takes the button itself
            async def option_button_callback(interaction: discord.Interaction, btn_ref=button):
                await self.option_callback(interaction, btn_ref)
            button.callback = option_button_callback
            self.add_item(button)
        ...
        submit_button = discord.ui.Button(
            ...
            custom_id=f"submit_approvals_{self.proposal_id}",
            disabled=True # Initially disabled until an option is selected
        )
        submit_button.callback = self.submit_approvals_callback
        self.add_item(submit_button)

    def has_selection(self) -> bool:
        return bool(self.current_approvals)

    def get_mechanism_vote_data(self) -> Dict[str, Any]:
        return {"approved": list(self.current_approvals)}

    async def option_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        # This callback is for individual approval option buttons.
        # It should NOT defer. It updates state and edits the message immediately.

        option_label = button.label
        if option_label in self.current_approvals:
            self.current_approvals.remove(option_label)
            button.style = discord.ButtonStyle.secondary # Deselected
        else:
            self.current_approvals.add(option_label)
            button.style = discord.ButtonStyle.success   # Selected

        # Update self.selected_mechanism_vote_data for BaseVoteView compatibility if it relies on it
        self.selected_mechanism_vote_data = self.get_mechanism_vote_data()

        # Enable/disable the actual "Submit Approvals" button based on selection
        # Find the submit button by its custom_id pattern
        actual_submit_button = discord.utils.find(lambda item: isinstance(item, discord.ui.Button) and item.custom_id.startswith("submit_approvals_"), self.children)
        if actual_submit_button:
            actual_submit_button.disabled = not self.has_selection()

        # Immediately edit the message to reflect button style changes and submit button state
        if not interaction.response.is_done():
            try:
            ...
            print(f"WARNING: ApprovalVoteView.option_callback: Interaction already responded to. Cannot edit. P#{self.proposal_id}")
    ...
    async def submit_approvals_callback(self, interaction: discord.Interaction):
        ...
        # self.current_approvals set by option_callbacks is used by get_mechanism_vote_data.

        if not self.has_selection():
            # This check might be redundant if the button is correctly disabled/enabled,
            # but good for safety.
            if not interaction.response.is_done():
        ...
        should_send_modal = (
            ...
            self.user_remaining_tokens is not None and
            self.user_remaining_tokens > 0
            # self.is_abstain_vote is False here
        )
        ...
        if not should_send_modal:
            if not interaction.response.is_done():
                ...
                print(f"WARNING: ApprovalVoteView.submit_approvals_callback trying to defer an already responded interaction. P#{self.proposal_id} U#{self.user_id}")

        # Disable all buttons after submission attempt
        for item in self.children:
            if isinstance(item, discord.ui.Button):
                item.disabled = True

        # If we deferred, edit the message to show updated (disabled) view state.
        if not should_send_modal:
            if interaction.response.is_done(): # Should be done if we deferred successfully
                try:
                    await interaction.edit_original_response(view=self)
                except discord.HTTPException as e:
                    print(f"Error editing message in ApprovalVoteView.submit_approvals_callback after defer: {e}")
