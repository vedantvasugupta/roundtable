    ...
    def count_votes(votes: List[Dict]):
        ...
        for vote in votes:
            ...
            if not isinstance(vote_data, dict):
                print(
                    f"WARNING: Expected dict for vote_data but got {type(vote_data)} in PluralityVoting.count_votes. Skipping vote: {vote}")
                continue

            option = vote_data.get('option')
            if option is None:  # Check explicitly for None
                print(
                    f"WARNING: 'option' key missing or None in vote_data for PluralityVoting. Skipping vote: {vote}")
                continue
            if not isinstance(option, str):  # Ensure option is a string
                print(
                    f"WARNING: Expected string for 'option' but got {type(option)} in PluralityVoting.count_votes. Skipping vote: {vote}")
                continue

            if option:  # Check if the string is non-empty after ensuring it's a string
        ...
        # Determine winner (highest count, handle ties if needed - simple tie goes to first listed)
...
async def calculate_results(mechanism_name: str, raw_votes: List[Dict], proposal: Dict) -> Dict:
    """
    Calculate voting results based on the mechanism.
    This function now filters abstain votes before passing to the mechanism's counter.
    """
    try:
        print(
            f"DEBUG: Calculating results for proposal #{proposal.get('proposal_id') or proposal.get('id')} using {mechanism_name}")
        # Separate abstain votes from non-abstain votes
        non_abstain_votes = []
        abstain_count = 0
        total_votes_cast = len(raw_votes)

        for vote in raw_votes:
            vote_data = vote.get('vote_data')
            # Ensure vote_data is a dictionary, handle potential JSON strings from older data or errors
            if isinstance(vote_data, str):
                try:
                     vote_data = json.loads(vote_data)
                except json.JSONDecodeError:
                    print(f"WARNING: Could not decode JSON vote_data for vote {vote.get('vote_id')}. Skipping.")
                    continue  # Skip this vote if data is corrupt
            elif not isinstance(vote_data, dict):
                print(f"WARNING: Invalid vote_data format for vote {vote.get('vote_id')}. Expected dict, got {type(vote_data)}. Skipping.")
                continue  # Skip this vote if data is wrong type

            if vote_data.get('did_abstain', False) is True:  # Check for the boolean True explicitly
                abstain_count += 1
            else:
                non_abstain_votes.append(vote)

        print(
            f"DEBUG: Total votes cast: {total_votes_cast}, Non-abstain votes: {len(non_abstain_votes)}, Abstain votes: {abstain_count}")

        # Get the mechanism-specific counter
        mechanism = get_voting_mechanism(mechanism_name)
        if not mechanism:
            print(
                f"ERROR: Invalid voting mechanism '{mechanism_name}' for calculation.")
            return {
                'mechanism': mechanism_name,
                 ...
                 'results': [],
                 'winner': None,
                 'details': f"Error: Invalid voting mechanism '{mechanism_name}'"
            }

        # Get the original options list for the proposal
        ...
        if not options:
            ...
            if not options:
                ...
                print(
                    f"WARNING: No options found for proposal {proposal_id} during calculation. Using default Yes/No.")

        # Calculate mechanism-specific results using only non-abstain votes
        # Pass options list to count_votes methods where needed (especially Borda, Approval, Runoff)
        # Modify count_votes signatures if necessary to accept options.
        # Let's adjust the count_votes methods in the classes above to accept options.
        # Re-checking the `count_votes` methods... they don't currently accept options.
        # For Plurality/DHondt, they don't strictly *need* the full list if they only count received votes.
        # For Borda/Runoff, calculating points/elimination based on *all* options on the ballot is standard.
        # For Approval, showing options with 0 approvals is standard.
        # Let's adjust `count_votes` methods to accept `all_options: List[str]` as the second argument.

        # Re-implement calling count_votes with options
        try:
            # Check mechanism name case-insensitively
            mechanism_name_lower = mechanism_name.lower()
            if mechanism_name_lower in ["plurality", "dhondt", "borda", "approval", "runoff"]:
                mechanism_results = mechanism.count_votes(non_abstain_votes) # Removed options argument for now, simpler implementation
                 # We will add options with 0 results during formatting if necessary
            else:
                # Should not happen if get_voting_mechanism worked
                print(f"ERROR: Unexpected mechanism name '{mechanism_name}' passed to calculate_results.")
                mechanism_results = {'results': [], 'winner': None}  # Default empty results

        except Exception as e:
            print(
                f"ERROR during mechanism count_votes for {mechanism_name}: {e}")
            import traceback
            traceback.print_exc()
            mechanism_results = {'results': [], 'winner': None}  # Return empty results on error

        # Combine mechanism results with abstain count
        final_results = {
            'mechanism': mechanism_name,
            'total_votes': total_votes_cast,
            'non_abstain_count': len(non_abstain_votes),
            'abstain_count': abstain_count,
            'results': mechanism_results.get('results', []),  # Mechanism-specific results
            'winner': mechanism_results.get('winner'),
            'rounds': mechanism_results.get('rounds'),  # For Runoff
            'top_quotients': mechanism_results.get('top_quotients')  # For DHondt
            # 'details': mechanism_results.get('details') # Details moved to format
        }

        print(
            f"DEBUG: Calculation complete for proposal #{proposal_id}. Results: {final_results}")
        return final_results

    except Exception as e:
        print(
            f"CRITICAL ERROR in calculate_results for proposal {proposal.get('proposal_id') or proposal.get('id')}: {e}")
        import traceback
        traceback.print_exc()
        return {  # Return error result
                 ...
                 'results': [],
                 'winner': None,
                 'details': f"Error during calculation: {str(e)}"
        }
...
async def close_proposal(proposal_id: int) -> Optional[Dict]:
    ...
    try:
        ...
        results = await calculate_results(mechanism_name, votes, proposal)
