...
class PluralityVoteView(BaseVoteView):
    ...
    def add_mechanism_items(self):
        self.selected_option_value: Optional[str] = None
        for i, option_text in enumerate(self.options):
            btn = discord.ui.Button(
                label=option_text[:80],
                style=discord.ButtonStyle.secondary,
                custom_id=f"plurality_{self.proposal_id}_{i}",
                row = i // 5 # Arrange in rows of 5
            )
            btn.callback = self.option_callback
            self.add_item(btn)

    def has_selection(self) -> bool:
        # This is called by BaseVoteView to check if a mechanism-specific choice was made
        return self.selected_option_value is not None

    # get_mechanism_vote_data is inherited from BaseVoteView if it just returns self.selected_mechanism_vote_data
    # If specific formatting is needed, override it.
    # For plurality, self.selected_mechanism_vote_data = {'option': value} is fine.

    async def option_callback(self, interaction: discord.Interaction):
        # BaseVoteView.interaction_check is implicitly called before this if it's a child of BaseVoteView
        # and the interaction is dispatched through the view.
        # However, an explicit check here can be a safeguard if this callback could be triggered otherwise.
        if not await self.interaction_check(interaction): # Use self.interaction_check as it's a method of BaseVoteView
            return

        custom_id = interaction.data["custom_id"]
        # Safely extract index from custom_id (e.g., "plurality_123_0")
        try:
            clicked_option_index = int(custom_id.split("_")[-1])
            if not (0 <= clicked_option_index < len(self.options)):
        ...
        self.selected_mechanism_vote_data = {'option': self.selected_option_value}
        self.is_abstain_vote = False

        # Update button styles and disable/enable relevant controls
        for item in self.children: # Iterate over children of this view
            if isinstance(item, discord.ui.Button):
                if item.custom_id == custom_id: # Clicked button
                    item.style = discord.ButtonStyle.primary
                    # item.disabled = True # Optionally disable after selection, or allow re-selection
                elif item.custom_id.startswith(f"plurality_{self.proposal_id}_"): # Other plurality option buttons
                    item.style = discord.ButtonStyle.secondary
                    # item.disabled = False # Keep other options enabled if re-selection is allowed before submit
                elif isinstance(item, AbstainButton): # Abstain button
                    item.style = discord.ButtonStyle.secondary
                    item.disabled = True # Disable abstain if an option is chosen

        self.submit_button.disabled = False # Enable submit once a choice is made
        await interaction.response.edit_message(view=self)
        # Optional: await interaction.followup.send(f"You selected: {self.selected_option_value}. Click 'Submit Vote'.", ephemeral=True)
...
class RankedVoteView(BaseVoteView):
    ...
    def _add_rank_select_menu(self):
        ...
        if not remaining_options:
            self.submit_button.disabled = not self.has_selection() # Enable submit if something is ranked
            return

        rank_num = len(self.current_ranking) + 1
        select_options = [discord.SelectOption(label=opt[:100], value=opt) for opt in remaining_options]
        if not select_options: # Should not happen if remaining_options is not empty, but safeguard
             self.submit_button.disabled = not self.has_selection()
             return

        select = discord.ui.Select(
            placeholder=f"Select your #{utils.get_ordinal_suffix(rank_num)} choice"[:100],
    ...
    # get_mechanism_vote_data is inherited, will use self.selected_mechanism_vote_data
    ...
    async def rank_callback(self, interaction: discord.Interaction):
        ...
        self.selected_mechanism_vote_data = {"rankings": self.current_ranking} # Update selected data
        self.is_abstain_vote = False

        abstain_btn = discord.utils.get(self.children, custom_id=lambda x: isinstance(x, str) and x.startswith(f"abstain_vote_{self.proposal_id}"))
        if abstain_btn and isinstance(abstain_btn, discord.ui.Button):
        ...
        await interaction.response.edit_message(view=self)
        # rank_status_msg = "Current ranking:\n" + "\n".join(f"{i+1}. {opt}" for i, opt in enumerate(self.current_ranking))
        # await interaction.followup.send(rank_status_msg[:2000], ephemeral=True)
...
class ApprovalVoteView(BaseVoteView):
    ...
    # get_mechanism_vote_data is inherited
    ...
    async def option_callback(self, interaction: discord.Interaction):
        ...
        if option_value in self.approved_values:
            ...
            if clicked_button and isinstance(clicked_button, discord.ui.Button): clicked_button.style = discord.ButtonStyle.secondary
        else:
            self.approved_values.append(option_value)
            if clicked_button and isinstance(clicked_button, discord.ui.Button): clicked_button.style = discord.ButtonStyle.primary

        self.selected_mechanism_vote_data = {"approved": self.approved_values} # Update selected data
        self.is_abstain_vote = False

        abstain_btn = discord.utils.get(self.children, custom_id=lambda x: isinstance(x, str) and x.startswith(f"abstain_vote_{self.proposal_id}"))
        if abstain_btn and isinstance(abstain_btn, discord.ui.Button):
            abstain_btn.disabled = self.has_selection()
            if self.has_selection(): abstain_btn.style = discord.ButtonStyle.secondary

        self.submit_button.disabled = not (self.has_selection() or self.is_abstain_vote)
        await interaction.response.edit_message(view=self)
        # approved_list_str_msg = f"Approved: {(', '.join(self.approved_values) if self.approved_values else 'None')}. Click 'Submit Vote'."
        # await interaction.followup.send(approved_list_str_msg[:2000], ephemeral=True)
