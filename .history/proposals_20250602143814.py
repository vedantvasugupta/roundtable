...
class CampaignControlView(discord.ui.View):
    ...
    async def update_button_states(self, interaction: Optional[discord.Interaction] = None):
        ...
        # Fetch all proposals for the campaign once to avoid multiple DB calls
        all_guild_proposals = []
        if campaign.get('guild_id'):
            all_guild_proposals = await db.get_server_proposals(campaign['guild_id'])

        proposals_in_campaign = [p for p in all_guild_proposals if p.get('campaign_id') == self.campaign_id]

        # Sort by scenario_order for easier processing
        ...
        if campaign['status'] == 'setup':
            ...
            # Check if there's any scenario that is approved (status 'Pending' after admin approval)
            approved_scenarios_for_start = [p for p in proposals_in_campaign if p.get('status') == 'Pending'] # Assuming 'Pending' is the status after admin approval for a scenario
