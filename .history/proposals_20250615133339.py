...
async def _create_new_proposal_entry(
    ...
    """Creates a new proposal entry in the database."""
    try:
        ...
        if proposal_id:
            # Proposal created successfully, now add options
            options = hyperparameters.get("options", ["Yes", "No"])
            await db.add_proposal_options(proposal_id, options)

            # Notify the user who submitted the proposal
            await interaction.followup.send(
                f"🗳️ Proposal '{title}' (ID: {proposal_id}) has been submitted for admin approval. "
                f"You will be notified once it's reviewed.",
                ephemeral=True
            )
            ...
            await interaction.followup.send("Failed to create the proposal in the database.", ephemeral=True)

    except Exception as e:
        print(f"Error in _create_new_proposal_entry: {e}")
        traceback.print_exc()
        await interaction.followup.send(f"An error occurred while creating the proposal: {e}", ephemeral=True)
...
async def _perform_approve_campaign_action(admin_interaction_for_message_edit: discord.Interaction, campaign_id: int, bot_instance: commands.Bot, admin_view_to_disable: AdminCampaignApprovalView):
    ...
    try:
        ...
        await admin_interaction_for_message_edit.followup.send(f"Warning: Campaign C#{campaign_id} approved, but error occurred while auto-approving its existing scenarios: {e_update_scenarios}", ephemeral=True)
    # --- END NEW ---
