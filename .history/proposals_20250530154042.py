...
class PluralityProposalModal(BaseProposalModal):
    def __init__(self, interaction: discord.Interaction, mechanism_name: str):
        ...
        # This is the 5th item total (4 from base + 1 here)

    async def on_submit(self, interaction: discord.Interaction):
        threshold_str = self.winning_threshold_percentage_input.value.strip()
        winning_threshold_percentage = None # Default to None (simple majority)
        ...
        # For Plurality, we are hardcoding allow_abstain to True for now.
        # This could be changed if a different default is desired or if UI space allows configuring it later.
...
class BordaProposalModal(BaseProposalModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if allow_abstain_str == 'no':
            allow_abstain = False
        elif allow_abstain_str == 'yes' or allow_abstain_str == '':
            allow_abstain = True
        else: # Invalid input
        ...
        specific_hyperparameters = {"allow_abstain": allow_abstain}
        await self.common_on_submit(interaction, specific_hyperparameters)
...
class DHondtProposalModal(BaseProposalModal):
    def __init__(self, interaction: discord.Interaction, mechanism_name: str):
        ...
        # For D'Hondt, we might also want allow_abstain. If so, it must be hardcoded or obtained differently.
        # For now, D'Hondt will only have num_seats configurable here.
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Hardcoding allow_abstain for D'Hondt for now, similar to Plurality. Needs review if configurable.
        specific_hyperparameters = {
            "num_seats": num_seats,
            "allow_abstain": True
        }
