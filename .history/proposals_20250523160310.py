...
async def start_proposal_creation_flow(ctx: commands.Context):
    ...
    try:
        ...
        invoker_id = ctx.author.id # Always use ctx.author.id for invoker_id

        view = ProposalMechanismSelectionView(original_interaction=initial_interaction, invoker_id=invoker_id)
        ...
        if initial_interaction:
            ...
            # Store the message on the view if we need to delete it later from the view or a button.
            # However, standard practice is that the view itself wouldn't delete its own host message without context.
            # view.message = message # Optional: if needed by view logic later
