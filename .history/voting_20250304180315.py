...
from voting_utils import (
    get_voting_mechanism, format_vote_results, close_proposal, 
    check_expired_proposals, close_and_announce_results, VotingMechanism
)
        ...
        if invited_voters and len(all_votes) >= len(invited_voters):
            ...
            if guild is None:
                ...
                try:
                    ...
                    print(f"[ERROR] Failed to announce results for proposal {proposal_id}: {e}")
    except Exception as e:
        print(f"[ERROR] Error checking if all voters have voted: {e}")
