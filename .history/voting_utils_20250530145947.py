...
async def format_vote_results(results: Dict, proposal: Dict) -> discord.Embed:
    ...
    if mechanism == 'plurality':
        total_votes = results.get('total_effective_votes', 0)
        ...
        if results.get('results'):
            ...
            for option, count in results.get('results'):
                ...
                results_text += f"• {option}: {count} votes ({percentage:.2f}%)\n"
    ...
    # Add total raw votes
    total_raw_votes = results.get('total_raw_votes', results.get('total_effective_votes', 0) + abstain_count)
    embed.add_field(name="Total Ballots Cast", value=str(total_raw_votes), inline=True)

    # Add total effective votes
    total_effective_votes = results.get('total_effective_votes', 0)
