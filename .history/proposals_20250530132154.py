...
async def approve_proposal(ctx, proposal_id: int):
    ...
    try:
        ...
        await ctx.send(f"✅ Proposal #{proposal_id} ('{proposal['title']}') has been approved and voting has started!")

        # Announce in proposals channel
        proposals_channel_name = "proposals" # Make sure this matches your CHANNELS dict in main.py
        proposals_channel = await utils.get_or_create_channel(ctx.guild, proposals_channel_name, ctx.bot.user.id)

        if proposals_channel:
            # Fetch options for the embed
            options = await db.get_proposal_options(proposal_id)
            option_names = [opt['option_text'] for opt in options] if options else ["Yes", "No"]

            embed = utils.create_proposal_embed(
                proposal_id,
                ctx.guild.get_member(proposal['proposer_id']) or proposal['proposer_id'], # Fetch member or use ID
                proposal['title'],
            ...
            await ctx.send(f"⚠️ Could not find the `{proposals_channel_name}` channel to announce the vote.")

        # Announce in voting-room and start tracking
        voting_room_channel_name = "voting-room" # Make sure this matches your CHANNELS dict in main.py
        voting_room_channel = await utils.get_or_create_channel(ctx.guild, voting_room_channel_name, ctx.bot.user.id)
        if voting_room_channel:
            # The update_vote_tracking function will send the initial tracking message.
            await voting_utils.update_vote_tracking(ctx.guild, proposal_id)
            await voting_room_channel.send(f"🗳️ Voting for Proposal #{proposal_id} ('{proposal['title']}') is now open. Check your DMs to vote or use the `!vote` command here (voting via DM is recommended for privacy).")
        else:
            await ctx.send(f"⚠️ Could not find the `{voting_room_channel_name}` channel.")


        # Log to audit channel
        audit_channel = discord.utils.get(ctx.guild.text_channels, name="audit-log")
        if audit_channel:
            await audit_channel.send(f"✅ **Proposal Approved**: #{proposal_id} ('{proposal['title']}') approved by {ctx.author.mention}. Voting started.")

    except Exception as e:
        print(f"Error in approve_proposal: {e}")
        traceback.print_exc()
        await ctx.send(f"❌ An error occurred while approving proposal #{proposal_id}: {e}")
...
async def reject_proposal(ctx, proposal_id: int, reason: str):
    ...
    try:
        ...
        if proposal['status'] not in ["Pending", "Pending Approval"]:
            await ctx.send(f"❌ Proposal #{proposal_id} cannot be rejected (current status: {proposal['status']}).")
            return

        # Update proposal status
        await db.update_proposal_status(proposal_id, "Rejected")
        await ctx.send(f"❌ Proposal #{proposal_id} ('{proposal['title']}') has been rejected. Reason: {reason}")

        # Notify proposer
        try:
            proposer = await ctx.guild.fetch_member(proposal['proposer_id'])
            if proposer:
                await proposer.send(f"Your proposal #{proposal_id} ('{proposal['title']}') was rejected by an administrator. Reason: {reason}")
        except discord.NotFound:
            print(f"Could not find proposer with ID {proposal['proposer_id']} to notify of rejection.")
        except discord.Forbidden:
            print(f"Could not DM proposer {proposal['proposer_id']} about rejection (DMs disabled or bot blocked).")


        # Log to audit channel
        audit_channel = discord.utils.get(ctx.guild.text_channels, name="audit-log")
        if audit_channel:
            await audit_channel.send(f"🚫 **Proposal Rejected**: #{proposal_id} ('{proposal['title']}') rejected by {ctx.author.mention}. Reason: {reason}")

    except Exception as e:
        print(f"Error in reject_proposal: {e}")
        traceback.print_exc()
        await ctx.send(f"❌ An error occurred while rejecting proposal #{proposal_id}: {e}")
