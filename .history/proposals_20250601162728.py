...
async def _create_new_proposal_entry(interaction: discord.Interaction, title: str, description: str, mechanism_name: str, options: List[str], deadline_db_str: str, hyperparameters: Optional[Dict[str, Any]] = None, campaign_id: Optional[int] = None, scenario_order: Optional[int] = None) -> Optional[int]:
    ...
    try:
        ...
        final_initial_status = None # Determine this based on context

        # Determine if proposal/scenario requires explicit admin approval for ITS OWN definition
        scenario_requires_separate_approval = True # Default assumption

        if campaign_id:
            campaign_status = campaign.get('status') if 'campaign' in locals() and campaign else None
            if campaign_status in ['setup', 'active']:
                # If campaign is already approved (setup) or running (active),
                # this new scenario for it does not need separate admin approval to be DEFINED.
                # Its status should be 'ApprovedScenario' so it's ready for the control panel.
                scenario_requires_separate_approval = False
                final_initial_status = "ApprovedScenario"
            # If campaign status is 'pending_approval', this scenario, if defined, will also be 'Pending Approval'
            # and will be actioned when the campaign itself is actioned.
            # So, scenario_requires_separate_approval remains True, and final_initial_status will become 'Pending Approval' below.

        # If still requires approval (standalone, or scenario for non-active campaign), check constitutional variables
        if scenario_requires_separate_approval:
            ...
            try:
                ...
                else: # Standalone and constitution says no approval needed
                    final_initial_status = "Voting"
            except Exception as e_cv:
                print(f"Notice: Could not fetch const_vars for proposal_requires_approval. Defaulting to Pending Approval. Error: {e_cv}")
                final_initial_status = "Pending Approval"

        # Fallback if final_initial_status wasn't set by specific logic paths above
        if final_initial_status is None:
            # This case implies scenario_requires_separate_approval was False (e.g. scenario for active campaign)
            # but final_initial_status wasn't explicitly set to ApprovedScenario. This should have been caught.
            # Defaulting to ApprovedScenario for safety if it's a campaign context, else Voting if standalone and no approval needed.
            if campaign_id:
                 final_initial_status = "ApprovedScenario" # Safety net
            else: # Standalone, and wasn't caught by const_vars logic above (should imply no approval needed)
                 final_initial_status = "Voting" # Safety net, implies const_vars said no approval
            print(f"WARN: final_initial_status was None for P#{proposal_id_for_debug}, C#{campaign_id}, S#{scenario_order}. Defaulted to {final_initial_status}")

        # The requires_approval flag for db.create_proposal is now more about whether it *went through* an approval check
        # rather than dictating its initial status directly, as final_initial_status now does that.
        # However, db.create_proposal uses requires_approval for its internal fallback if initial_status is None,
        # so ensure it reflects whether approval would have been needed if not for campaign context.
        # Let's simplify: if final_initial_status is 'Pending Approval', then requires_approval is True. Otherwise False.
        ...
        proposal_id = await db.create_proposal(
            ...
            requires_approval=db_requires_approval_flag, # Informational for DB, status is king
            hyperparameters=hyperparameters,
            campaign_id=campaign_id,
            scenario_order=scenario_order,
            initial_status=final_initial_status # Explicitly set the status
        )
        ...
        # Make sure to update the CampaignControlView if a scenario was added to an existing campaign.
        if campaign_id and scenario_order and final_initial_status == "ApprovedScenario":
            campaign_data = await db.get_campaign(campaign_id)
            if campaign_data and campaign_data.get('control_message_id') and interaction.guild: # Ensure guild context for fetching message
                try:
                    ...
                    if campaign_mgmt_channel:
                        control_message = await campaign_mgmt_channel.fetch_message(campaign_data['control_message_id'])
                        if control_message:
                            # Create a new view instance to refresh its state
                            updated_view = CampaignControlView(campaign_id, interaction.client) # Use interaction.client as bot instance
                            await updated_view.update_button_states() # This method recalculates button states

                            # Also update the embed to reflect new scenario count etc.
                            refetched_campaign_data = await db.get_campaign(campaign_id)
                            creator = await interaction.guild.fetch_member(refetched_campaign_data['creator_id'])
                            embed_title = f"Campaign Management: '{refetched_campaign_data['title']}' (ID: C#{campaign_id})"
                            embed_desc = f"**Creator:** {creator.mention if creator else f'ID: {refetched_campaign_data['creator_id']}'}\n"
                            embed_desc += f"**Description:** {refetched_campaign_data['description'] or 'Not provided.'}\n"
                            embed_desc += f"**Total Scenarios Expected:** {refetched_campaign_data['num_expected_scenarios']}\n"
                            embed_desc += f"**Currently Defined:** {refetched_campaign_data['current_defined_scenarios']}"
                            updated_embed = discord.Embed(title=embed_title, description=embed_desc, color=discord.Color.blue())
                            updated_embed.add_field(name="Status", value=refetched_campaign_data['status'].title(), inline=True)
                            updated_embed.add_field(name="Last Action Result", value=f"Scenario {scenario_order} defined.", inline=False)
