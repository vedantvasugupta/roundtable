...
async def create_proposal(server_id, proposer_id, title, description, voting_mechanism, deadline, requires_approval=True, hyperparameters: Optional[Dict[str, Any]] = None):
    ...
    async with get_db() as conn:
        cursor = await conn.execute(
            ...
            (server_id, guild_id, proposer_id, title, description, proposal_text, voting_mechanism, deadline, requires_approval, status, hyperparameters)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                server_id,  # server_id column
                server_id,  # guild_id column (using same value as server_id)
                proposer_id,
                title,
                description,
                description,  # proposal_text column (using same value as description)
                voting_mechanism,
                deadline,
                requires_approval,
                "Pending" if requires_approval else "Voting",
                hyperparameters_json # Store JSON string
            )
        )
        await conn.commit()
...
async def record_vote(user_id, proposal_id, vote_data):
    """Record or update a user's vote"""
    vote_json = json.dumps(vote_data)
    async with get_db() as conn:
        # Check if user has already voted
        async with conn.execute(
            "SELECT * FROM votes WHERE voter_id = ? AND proposal_id = ?",
            (user_id, proposal_id)
        ) as cursor:
        ...
        if existing_vote:
            ...
            await conn.execute(
                "UPDATE votes SET vote_data = ?, timestamp = CURRENT_TIMESTAMP WHERE voter_id = ? AND proposal_id = ?",
                (vote_json, user_id, proposal_id)
            )
        else:
            # Create new vote
            await conn.execute(
                "INSERT INTO votes (voter_id, proposal_id, vote_data, timestamp) VALUES (?, ?, ?, CURRENT_TIMESTAMP)",
                (user_id, proposal_id, vote_json)
            )
        await conn.commit()
...
async def init_db() -> None:
    ...
    async with get_db() as conn:
        # Enable foreign keys if not enabled by default by aiosqlite version
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS settings (
                ...
                server_id INTEGER NOT NULL UNIQUE, -- Changed from (setting_key, server_id) to make server_id unique for settings map
                key TEXT NOT NULL,
                value TEXT NOT NULL,
                FOREIGN KEY (server_id) REFERENCES servers(server_id) ON DELETE CASCADE
            )
        """)
        # Ensure UNIQUE constraint is on (server_id, key) if not already handled by UNIQUE on server_id and primary key on setting_id
        # For simplicity, assuming server_id UNIQUE is sufficient for the intended settings map logic

        await conn.execute(CREATE_CAMPAIGNS_TABLE) # Added earlier, ensure it uses conn
        await conn.execute(CREATE_USER_CAMPAIGN_PARTICIPATION_TABLE) # Added earlier

        await conn.execute("""
            CREATE TABLE IF NOT EXISTS proposals (
                proposal_id INTEGER PRIMARY KEY AUTOINCREMENT,
                server_id INTEGER NOT NULL,
                guild_id INTEGER, -- Keeping for now, might be redundant with server_id
                proposer_id INTEGER NOT NULL,
                title TEXT NOT NULL,
                description TEXT,
                proposal_text TEXT, -- Potentially longer/formatted description
                voting_mechanism TEXT NOT NULL,
                deadline TEXT NOT NULL, -- Store as ISO 8601 string
                ...
                approved_by INTEGER,
                creation_timestamp TEXT NOT NULL, -- Store as ISO 8601 string
                last_updated_timestamp TEXT NOT NULL, -- Store as ISO 8601 string
                ...
                voting_message_id TEXT, -- Legacy, consider removing or consolidating with vote_tracking_message_id
                vote_count INTEGER DEFAULT 0, -- Legacy, can be derived
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS proposal_options (
                ...
                option_order INTEGER, -- For ordered preferences if needed by some mechanisms
                FOREIGN KEY (proposal_id) REFERENCES proposals(proposal_id) ON DELETE CASCADE,
                UNIQUE (proposal_id, option_text) -- Ensure options are unique per proposal
            )
        """)
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS votes (
                ...
                proposal_id INTEGER NOT NULL,
                user_id INTEGER NOT NULL, -- Changed from voter_id for consistency
                ...
                UNIQUE (proposal_id, user_id) -- Typically one vote per user per proposal
            )
        """)
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS constitutional_variables (
                ...
                server_id INTEGER NOT NULL,
                name TEXT NOT NULL, -- Renamed from variable_name
                value TEXT NOT NULL, -- Renamed from variable_value
                type TEXT NOT NULL, -- Renamed from variable_type
                description TEXT,
        ...
        await conn.execute(CREATE_PENDING_PROPOSAL_NOTIFICATIONS_TABLE) # Already defined

        # Ensure columns after all tables are created to avoid issues with FKs to tables not yet existing
        # For proposals table
        ...
        await _ensure_column(conn, 'proposals', 'creation_timestamp TEXT')
        await _ensure_column(conn, 'proposals', 'last_updated_timestamp TEXT')
        ...
        await _ensure_column(conn, 'voting_invites', 'timestamp TEXT')

        # For constitutional_variables table
        await _ensure_column(conn, 'constitutional_variables', 'last_updated_timestamp TEXT')

        # For new tables
        await _ensure_column(conn, 'campaigns', 'creation_timestamp TEXT')
        await _ensure_column(conn, 'user_campaign_participation', 'last_updated_timestamp TEXT')

        await conn.commit()
        print("Database initialized and tables created/verified.")

    # Fix timestamps if necessary (run once or as needed)
    # await fix_all_timestamps() # Be cautious with this
    print("DB Initialization and column checks complete.")

# --- Data Modification and Retrieval Functions ---

    # Fix any malformed timestamps in the database
    await fix_malformed_timestamps()
