...
async def fix_malformed_timestamps():
    ...
    async with get_db() as conn:
        ...
        async with conn.execute("SELECT proposal_id, created_at, deadline FROM proposals") as cursor:
            proposals = await cursor.fetchall()
            for proposal in proposals:
                ...
                if created_at and isinstance(created_at, str) and 'T' in created_at:
                    ...
                    try:
                        ...
                        await conn.execute(
                            "UPDATE proposals SET created_at = ? WHERE proposal_id = ?",
                ...
                if deadline and isinstance(deadline, str) and 'T' in deadline:
                    ...
                    try:
                        ...
                        new_timestamp = dt.strftime('%Y-%m-%d %H:%M:%S')
                        await conn.execute(
                            "UPDATE proposals SET deadline = ? WHERE proposal_id = ?",
                            (new_timestamp, proposal_id)
                        )
        ...
        # Check votes table
        async with conn.execute("SELECT vote_id, timestamp FROM votes") as cursor:
            ...
            for vote in votes:
                ...
                if timestamp and isinstance(timestamp, str) and 'T' in timestamp:
                    try:
                        ...
                        new_timestamp = dt.strftime('%Y-%m-%d %H:%M:%S')
                        await conn.execute(
                            "UPDATE votes SET timestamp = ? WHERE vote_id = ?",
        ...
        async with conn.execute("SELECT warning_id, timestamp FROM warnings") as cursor:
            ...
            for warning in warnings:
                ...
                if timestamp and isinstance(timestamp, str) and 'T' in timestamp:
                    try:
                        ...
                        new_timestamp = dt.strftime('%Y-%m-%d %H:%M:%S')
                        await conn.execute(
                            "UPDATE warnings SET timestamp = ? WHERE warning_id = ?",
        ...
        # Check temp_moderation table
        async with conn.execute("SELECT action_id, expires_at FROM temp_moderation") as cursor:
            ...
            for action in actions:
                ...
                if expires_at and isinstance(expires_at, str) and 'T' in expires_at:
                    try:
                        ...
                        new_timestamp = dt.strftime('%Y-%m-%d %H:%M:%S')
                        await conn.execute(
                            "UPDATE temp_moderation SET expires_at = ? WHERE action_id = ?",
