...
async def _create_new_proposal_entry(interaction: discord.Interaction, title: str, description: str, mechanism_name: str, options: List[str], deadline_db_str: str, hyperparameters: Optional[Dict[str, Any]] = None, campaign_id: Optional[int] = None, scenario_order: Optional[int] = None) -> Optional[int]:
    ...
    try:
        ...
        # MINIMAL USER FEEDBACK FOR THIS STEP - to be expanded later
        user_feedback = f"✅ P#{proposal_id} ('{title}') created with status: {final_initial_status}."
        if final_initial_status == "Pending Approval": user_feedback += " Needs admin approval."
        await interaction.followup.send(user_feedback, ephemeral=False) # Basic non-ephemeral feedback

        # Admin notification and CampaignControlView update logic is OMITTED for this focused edit.
        return proposal_id
