...
class CampaignControlView(discord.ui.View):
    ...
    async def start_next_callback(self, interaction: discord.Interaction):
        ...
        if campaign['status'] == 'setup':
            # Find all approved scenarios for order 1
            scenarios_to_start_ids = [
                p['proposal_id'] for p in proposals_in_campaign
                if p.get('scenario_order') == 1 and p.get('status') == 'ApprovedScenario'
            ]
            ...
            if not scenarios_to_start_ids:
                ...
                # Initiate voting for all found scenarios for order 1
                ...
                if success_init_stage:
                    action_taken_message += f"Processing for {len(scenarios_to_start_ids)} scenario(s) at order 1 initiated. Details: {msg_init_stage}"
                else:
                    action_taken_message += f"Failed to initiate some/all scenarios at order 1. Details: {msg_init_stage}"
                    action_error = True

                audit_channel = discord.utils.get(guild.text_channels, name="audit-log")
                if audit_channel:
                    await audit_channel.send(f"▶️ **Campaign Started**: C#{self.campaign_id} ('{campaign['title']}') started by {interaction.user.mention}. {len(scenarios_to_start_ids)} scenario(s) for order 1 initiated.")
