    ...
    try:
        ...
        if campaign_id:
            ...
            if campaign and campaign.get('guild_id'):
                ...
                error_msg_campaign = f"ERROR: Could not retrieve guild_id from campaign C#{campaign_id}. Campaign data: {campaign}"
                print(error_msg_campaign)
                if interaction.guild_id is None:
                    await interaction.followup.send("Critical Error: Could not determine server for campaign scenario. Campaign data may be corrupt.", ephemeral=True)
                    return None
                server_id_to_use = interaction.guild_id
                if server_id_to_use is None:
                     await interaction.followup.send("Critical Error: Could not determine server for scenario (invalid campaign data & DM context).", ephemeral=True)
                     return None
                guild_for_logging = interaction.guild.name if interaction.guild else f"GuildID {server_id_to_use} (fallback)"
                print(f"WARN: Falling back to interaction.guild_id {server_id_to_use} (Guild: {guild_for_logging}) for scenario of C#{campaign_id}.")
        else:
            server_id_to_use = interaction.guild_id
            if server_id_to_use is None:
                print(f"ERROR: server_id_to_use is None for a standalone proposal (DM interaction).")
        ...
        requires_approval_check_done = False

        if campaign_id and campaign: # Ensure campaign object exists
