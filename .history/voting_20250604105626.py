    ...
    async def submit_vote_callback(self, interaction: discord.Interaction):
        ...
        if self.is_abstain_vote: # Handled by AbstainButton's callback directly calling finalize_vote
            ...
            if self.user_remaining_tokens == 0: # No tokens left, auto-invest 0
                 if not interaction.response.is_done(): # Acknowledge interaction if Plurality didn't edit
                    await interaction.response.defer(ephemeral=True, thinking=True)
                 await self.finalize_vote(interaction, tokens_invested_this_scenario=0)
            else:
                # Sending a modal is a direct response. No prior deferral needed here from submit_vote_callback.
                # Plurality option_callback no longer responds, so this send_modal will be the first response.
...
class PluralityVoteView(BaseVoteView):
    ...
    async def option_callback(self, interaction: discord.Interaction):
        # interaction_check is handled by the view's interaction_check method before dispatch

        button_label = interaction.data.get('label') # Fallback if custom_id parsing fails for label
        custom_id = interaction.data.get('custom_id')

        if custom_id:
            # Assuming custom_id like "plurality_option_PROPOSALID_INDEX" or "plurality_option_PROPOSALID_OPTIONLABEL"
            # For robustness, it's better if the button's label is directly used or stored on the button object
            # If options can have underscores, splitting by '_' and taking last part for index/label might be fragile.
            # Let's assume button.label is the option text, which is more direct.
            # The button object itself isn't directly passed to this callback in the same way it was before
            # when we did `button.callback = self.option_callback`.
            # We need to find the button that was clicked from self.children or rely on custom_id or label.
            # For simplicity, let's assume interaction.data['custom_id'] or finding the button by custom_id is reliable.
            # Or even better, the label of the button that was clicked should be accessible.
            # If using `discord.ui.Button.from_custom_id(interaction.client, interaction.data['custom_id'])` is too complex,
            # let's assume the label is sent in interaction.data if the component has one.
            # However, `interaction.data` for button clicks usually gives `custom_id` and `component_type`.
            # The label isn't directly in `interaction.data`.
            # The most reliable way is to find the component in `self.children` using the `custom_id`.
            clicked_component = discord.utils.find(lambda comp: hasattr(comp, 'custom_id') and comp.custom_id == custom_id, self.children)
            if clicked_component and hasattr(clicked_component, 'label'):
                self.selected_option = clicked_component.label
            else:
                # Fallback or error - this part is tricky without knowing how options are mapped to buttons
                # For now, if label is not on component, try to extract from custom_id if it was an index
                try:
                    # This assumes custom_id format like "plurality_option_PROPOSALID_INDEX"
                    option_idx = int(custom_id.split('_')[-1])
                    if 0 <= option_idx < len(self.options):
                        self.selected_option = self.options[option_idx]
                    else:
                        print(f"ERROR: Invalid option index {option_idx} from custom_id {custom_id}")
                        # If interaction not responded, send error
                        if not interaction.response.is_done():
                            await interaction.response.send_message("Error processing your selection.", ephemeral=True)
                        else:
                            await interaction.followup.send("Error processing your selection.", ephemeral=True)
                        return
                except (ValueError, IndexError):
                    print(f"ERROR: Could not determine selected option from custom_id {custom_id}")
                    if not interaction.response.is_done():
                         await interaction.response.send_message("Error identifying your selection.", ephemeral=True)
                    else:
                        await interaction.followup.send("Error identifying your selection.", ephemeral=True)
                    return

        else: # Should not happen if buttons have custom_ids
            print(f"ERROR: No custom_id found in interaction data for PluralityVoteView. P#{self.proposal_id}")
            if not interaction.response.is_done():
                await interaction.response.send_message("Interaction error.", ephemeral=True)
            else:
                await interaction.followup.send("Interaction error.", ephemeral=True)
            return

        # Conditional deferral logic
        # A modal is sent if it's a campaign, user has tokens, and it's not an abstain vote.
        # self.is_abstain_vote is set by the AbstainButton callback. Here it's false.
        should_send_modal = (
            self.campaign_id is not None and
            self.user_remaining_tokens is not None and
            self.user_remaining_tokens > 0
        )

        if not should_send_modal:
            if not interaction.response.is_done():
                await interaction.response.defer(ephemeral=True, thinking=True)
            else:
                print(f"WARNING: PluralityVoteView.option_callback trying to defer already responded interaction. P#{self.proposal_id} U#{self.user_id}")
        # If should_send_modal is True, we do *not* defer here, because
        # submit_vote_callback will call interaction.response.send_modal()

        # Update button styles to show selection (visual feedback)
        for item in self.children:
            if isinstance(item, discord.ui.Button) and hasattr(item, 'label'):
                if item.label == self.selected_option:
                    item.style = discord.ButtonStyle.success  # Mark selected
                else:
                    item.style = discord.ButtonStyle.secondary # Reset others
                item.disabled = True # Disable all option buttons after one is clicked

        # If we deferred, we need to edit the original message to show updated buttons
        # If we are going to send a modal, the modal is the response, so no edit here.
        if not should_send_modal:
            try:
                # This edit is for the button style changes if we deferred.
                # The final confirmation comes from finalize_vote.
                if interaction.response.is_done(): # Must be done if we deferred
                    await interaction.edit_original_response(view=self)
            except discord.HTTPException as e:
                print(f"Error editing message in PluralityVoteView.option_callback after defer: {e}")


        # Proceed to the general submission logic
...
class RankedVoteView(BaseVoteView):
    ...
    async def rank_callback(self, interaction: discord.Interaction):
        ...
        # Conditional deferral logic, similar to PluralityVoteView
        should_send_modal = (
            self.campaign_id is not None and
            self.user_remaining_tokens is not None and
            self.user_remaining_tokens > 0
            # Assuming self.is_abstain_vote is False here, as this is a rank_callback
        )
        ...
        if not should_send_modal:
            if not interaction.response.is_done():
                ...
                print(f"WARNING: RankedVoteView.rank_callback trying to defer already responded interaction. P#{self.proposal_id} U#{self.user_id}")

        if not should_send_modal:
            try:
                if interaction.response.is_done():
                     await interaction.edit_original_response(view=self) # Update view to show disabled select
            except discord.HTTPException as e:
                print(f"Error editing message in RankedVoteView.rank_callback after defer: {e}")
...
class ApprovalVoteView(BaseVoteView):
    ...
    async def option_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        # User check and submission check handled by BaseVoteView

        option_label = button.label
        if option_label in self._approved_options:
            self._approved_options.remove(option_label)
            button.style = discord.ButtonStyle.secondary # Deselected
        else:
            self._approved_options.append(option_label)
            button.style = discord.ButtonStyle.success # Selected

        # Update self.selected_mechanism_vote_data for has_selection() and get_mechanism_vote_data()
        self.selected_mechanism_vote_data = {"approved": list(self._approved_options)}

        # Enable/disable submit button based on selection
        submit_button = discord.utils.find(lambda i: isinstance(i, discord.ui.Button) and i.label == "Submit Approvals", self.children)
        if submit_button:
            submit_button.disabled = not self.has_selection()

        # For approval, the interaction is on a specific option button.
        # We don't defer here immediately because the user can click multiple options.
        # The deferral or modal send will happen when they click "Submit Approvals"
        # So, just edit the message to reflect button style changes.
        if not interaction.response.is_done():
            await interaction.response.edit_message(view=self)
        else:
            # This case should ideally not happen if interaction_check is effective.
            # If it does, we can't edit. Perhaps log and do nothing.
            print(f"WARN: ApprovalVoteView.option_callback: Interaction already responded to. Cannot edit button styles. P#{self.proposal_id}")


    async def submit_approvals_callback(self, interaction: discord.Interaction):
        # This is called when the "Submit Approvals" button is clicked.
        # self.selected_mechanism_vote_data is already up-to-date from option_callbacks.

        if not self.has_selection():
            if not interaction.response.is_done():
                await interaction.response.send_message("Please select at least one option to approve.", ephemeral=True)
            else:
                await interaction.followup.send("Please select at least one option to approve.", ephemeral=True)
            return
        ...
        if not should_send_modal:
            if not interaction.response.is_done():
                ...
                print(f"WARNING: ApprovalVoteView.submit_approvals_callback trying to defer already responded interaction. P#{self.proposal_id} U#{self.user_id}")
        ...
        if not should_send_modal:
            try:
                if interaction.response.is_done(): # Must be done if we deferred
                    await interaction.edit_original_response(view=self) # Update view to show disabled buttons
            except discord.HTTPException as e:
                print(f"Error editing message in ApprovalVoteView.submit_approvals_callback: {e}")

        await self.submit_vote_callback(interaction) # This will then call finalize_vote or the modal
