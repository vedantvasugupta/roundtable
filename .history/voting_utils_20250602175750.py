...
async def format_vote_results(results: Dict, proposal: Dict) -> discord.Embed:
    """Format the results of a vote into a Discord embed, reflecting token weighting and detailed mechanism outputs."""
    ...
    elif results.get('total_raw_ballots', 0) == 0 and results.get('num_abstain_votes', 0) == 0:
        color = discord.Color.greyple()
        status_message = "No votes were cast."
    else: # Other no-winner scenarios or just displaying results
        status_message = "Voting concluded. See details below."

    embed = discord.Embed(
        title=f"Results: P#{proposal_id} - {title}",
        description=status_message,
        color=color
    )
    embed.add_field(name="Voting Method", value=mechanism.title(), inline=True)

    # Common fields: Total Ballots/Voters, Weighted Power, Abstains
    total_raw_voters_key = 'total_raw_ballots' # Default for Plurality, DHondt, Runoff
    if mechanism == 'borda': total_raw_voters_key = 'total_raw_vote_sets'
    elif mechanism == 'approval': total_raw_voters_key = 'total_raw_voters'

    total_weighted_power_key = 'total_weighted_votes' # Plurality
    if mechanism == 'borda': total_weighted_power_key = 'total_weighted_vote_power'
    elif mechanism == 'approval': total_weighted_power_key = 'total_weighted_voting_power'
    elif mechanism == 'runoff': total_weighted_power_key = 'total_weighted_ballot_power'
    elif mechanism == 'dhondt': total_weighted_power_key = 'total_weighted_vote_power'

    raw_ballots_cast = results.get(total_raw_voters_key, 0)
    weighted_power_cast = results.get(total_weighted_power_key, 0)
    abstain_count = results.get('num_abstain_votes', 0)
    tokens_in_abstain = results.get('tokens_in_abstain_votes', 0)

    embed.add_field(name="Ballots Cast (Non-Abstain)", value=str(raw_ballots_cast), inline=True)
    embed.add_field(name="Total Weighted Vote Power", value=f"{weighted_power_cast:.2f}", inline=True)
    embed.add_field(name=f"Abstain Votes ({tokens_in_abstain} tokens)", value=str(abstain_count), inline=True)

    # Mechanism-specific display logic
    if mechanism == 'plurality':
        if 'winning_threshold_percentage' in hyperparameters and hyperparameters['winning_threshold_percentage'] is not None:
            embed.add_field(name="Winning Threshold", value=f"{hyperparameters['winning_threshold_percentage']}% of weighted votes", inline=True)
        else:
            embed.add_field(name="Winning Threshold", value="Simple Majority (>50%) of weighted votes", inline=True)

        results_text = ""
        if results.get('results_detailed'):
            for option, details in results['results_detailed']:
                percentage = (details['weighted_votes'] / weighted_power_cast) * 100 if weighted_power_cast > 0 else 0
                results_text += f"• **{option}**: {details['weighted_votes']:.2f} weighted votes ({details['raw_votes']} raw) - {percentage:.2f}%\n"
        results_text = results_text or "No effective votes recorded."
        embed.add_field(name="Vote Counts (Weighted)", value=results_text, inline=False)

    elif mechanism == 'borda':
        results_text = ""
        if results.get('results_detailed'):
            for option, details in results['results_detailed']:
                results_text += f"• **{option}**: {details['weighted_score']:.2f} weighted score ({details['raw_score']} raw score)\n"
        results_text = results_text or "No effective rankings recorded."
        embed.add_field(name="Borda Scores (Weighted)", value=results_text, inline=False)

    elif mechanism == 'approval':
        results_text = ""
        if results.get('results_detailed'):
            for option, details in results['results_detailed']:
                percentage_of_power = (details['weighted_approvals'] / weighted_power_cast) * 100 if weighted_power_cast > 0 else 0
                results_text += f"• **{option}**: {details['weighted_approvals']:.2f} weighted approvals ({details['raw_approvals']} raw) - Approved by {percentage_of_power:.2f}% of total weighted power\n"
        results_text = results_text or "No effective approvals recorded."
        embed.add_field(name="Approval Counts (Weighted)", value=results_text, inline=False)

    elif mechanism == 'runoff':
        embed.add_field(name="Rounds Conducted", value=str(len(results.get('rounds_detailed', []))), inline=True)
        for i, round_data in enumerate(results.get('rounds_detailed', [])):
            round_text = f"**Round {round_data['round_number']}** (Active: {len(round_data.get('active_options_in_round',[]))})\n"
            sorted_options_this_round = sorted(round_data['weighted_votes_per_option'].items(), key=lambda x:x[1], reverse=True)
            for option, Wcount in sorted_options_this_round:
                Rcount = round_data['raw_ballots_per_option'].get(option,0)
                round_text += f"  • **{option}**: {Wcount:.2f} weighted votes ({Rcount} raw)\n"
            if i < 2 : # Show details for first 2 rounds, then summarize if too many
                embed.add_field(name=f"Round {round_data['round_number']} Details", value=round_text, inline=False)
            elif i == 2:
                 embed.add_field(name="Further Rounds", value="Details for subsequent rounds omitted for brevity if many.", inline=False)
        if not results.get('rounds_detailed'):
            embed.add_field(name="Rounds", value="No rounds processed (e.g., no valid ballots).", inline=False)

    elif mechanism == 'dhondt':
        num_seats_configured = results.get('num_seats_configured', 1)
        embed.add_field(name="Seats to Allocate", value=str(num_seats_configured), inline=True)
        seats_allocated = results.get('seats_allocated_detailed', {})
        allocation_text = ""
        if seats_allocated:
            for option, num_seats in seats_allocated.items():
                if num_seats > 0:
                    party_total_w = results.get('party_totals_detailed', {}).get(option, {}).get('weighted_votes', 0)
                    party_total_r = results.get('party_totals_detailed', {}).get(option, {}).get('raw_votes', 0)
                    allocation_text += f"• **{option}**: {num_seats} seat(s) (Total: {party_total_w:.2f} weighted / {party_total_r} raw votes)\n"
        allocation_text = allocation_text or "No seats allocated."
        embed.add_field(name="Seat Allocation", value=allocation_text, inline=False)

        # Optionally, show first few quotient rounds if needed for transparency, but can be verbose
        # For now, direct seat allocation summary is primary.

    # Add proposal link (placeholder URL)
    # proposal_url = f"https://yourserver.com/proposals/{proposal_id}" # Replace with actual URL if available
    # embed.add_field(name="View Proposal", value=f"[View Proposal]({proposal_url})", inline=False)

    embed.set_footer(text=f"Results for P#{proposal_id} | Mechanism: {mechanism.title()} | Calculated at {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}")
    return embed
