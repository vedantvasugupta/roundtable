...
async def test_mixed_environment():
    ...
    if updated_campaign['status'] != 'completed':
        # Campaign not yet completed, try to complete it
        ...
        print("✅ Campaign already completed in previous step")
    
    # Verify campaign status is completed
    final_campaign = await db.get_campaign(campaign_id)
    assert final_campaign['status'] == 'completed', f"Campaign status should be 'completed', got '{final_campaign['status']}'"
