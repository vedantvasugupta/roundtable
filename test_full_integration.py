...
async def test_full_integration():
    ...
    for i, scenario_id in enumerate(scenarios, 1):
        ...
        # Debug output for runoff voting
        if i == 4:  # Scenario 4 is runoff
            print(f"DEBUG: Runoff results structure: {results.keys()}")
            print(f"DEBUG: Full runoff results: {results}")
        ...
        # Special handling for runoff voting which has different structure
        if results['mechanism'] == 'runoff':
            # Runoff doesn't have 'results_detailed', it has 'rounds_detailed'
            assert 'rounds_detailed' in results, f"Missing rounds_detailed in runoff scenario {i} results"
        else:
        ...
        if results['mechanism'] == 'plurality':
            ...
            # Runoff has different structure - show final round results
            if results.get('rounds_detailed'):
                final_round = results['rounds_detailed'][-1]
                print(f"  Final round ({final_round['round_number']}):")
                for option, weighted_votes in final_round['weighted_votes_per_option'].items():
                    raw_votes = final_round['raw_ballots_per_option'].get(option, 0)
                    print(f"    • {option}: {weighted_votes} weighted votes ({raw_votes} raw ballots)")
            else:
                print(f"  Rounds completed: {len(results.get('rounds_detailed', []))}")
    ...
    # Trigger completion check for stress test campaign
    stress_completion = await voting_utils.check_and_announce_campaign_completion(guild, stress_campaign_id)
    if not stress_completion:
        # Manually complete if not auto-completed
        await db.update_campaign_status(stress_campaign_id, "completed")
    ...
    for campaign in all_campaigns:
        if campaign['status'] != 'completed':
            print(f"⚠️  Campaign {campaign['campaign_id']} status: {campaign['status']} - attempting completion")
            # Try to complete the campaign
            completion_result = await voting_utils.check_and_announce_campaign_completion(guild, campaign['campaign_id'])
            if not completion_result:
                # Force completion if needed
                await db.update_campaign_status(campaign['campaign_id'], "completed")
            # Re-fetch to verify
            updated_campaign = await db.get_campaign(campaign['campaign_id'])
            assert updated_campaign['status'] == 'completed', f"Campaign {campaign['campaign_id']} should be completed after manual completion"
        else:
            print(f"✅ Campaign {campaign['campaign_id']} already completed")
